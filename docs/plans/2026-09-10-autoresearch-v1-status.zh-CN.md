...
| V1-03 整体 | 进行中 | A1 关闭同控制器共享配额被完成记账清除/延长，B 关闭持久需求计数及已接线消费者，C 关闭已有预算准入的并发/故障边界，D 关闭原生结果分类和共享 inbox 误归属；可选角色组仍待收口。按主机保存不等于跨主机共享配额或逐结果同步持久化；所有 provider 的结构化 refusal/truncation 尚未集成 |
