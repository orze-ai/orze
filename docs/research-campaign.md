...
模型任务必须与实际选择器一致：目标、同分处理、次级目标，以及何种结果能够改变选择，都应明确。
如果有效分类错误率恰为0，选择只接受严格改善且同分保留旧候选，当前目标下的进一步搜索无法换选；项目可以直接执行必要确认。
[实际两项CPU对照](plans/2026-09-17-research-edits.zh-CN.md)验证了相同配置和完整留出预测，省去每项8次追加拟合与2次模型调用。
这是有证明条件的局部工作流，不是独立泛化、科学收敛或产品研究总期限；需要稳健性等另一个目标时，应另定可执行评价。
可用 [MinimumLoss 示例](../examples/research_comparison/loss_selection.py)统一生成模型说明、选择候选和检查精确下界。它要求输入已核验的有效开发结果，不能用观测最小值冒充理论下界；必要确认仍执行。[24项新划分对照与两项示例执行](plans/2026-09-17-research-selection.zh-CN.md)同时报告省下的搜索和留出退化，未设为默认停止策略。
