...
Pro 接受并追加提案不等于原生队列已经准入。同一执行配置以另一提案 ID 再次出现时，
正常去重可能将它留在 `ideas.md`，不会产出新候选。入口只在核对保留原文、当时数据库
快照及规范化的执行配置身份后，将它记为 `campaign.retained_duplicates` 并继续后续评价／轮次。
重复提案不增加科学观察或分数，其调用和等待成本仍计入。无法证明重复、已准入却缺少
产物、原生命令失败等情况仍终止该样本；核验也会重算这些区别。
...
需要核对逐样本比较时，可通过原生 CPU 动作使用[可选配对分析工具](paired-comparisons.md)。
它给出实际配置差异、完整配对状态、样本与组等权分差，以及提供的历史使用记录的重合量。
[真实模型诊断](plans/2026-09-17-research-paired-tool.zh-CN.md)支持计算辅助事实读取的局部收益；
后续方案仍存在研究错误，不将这种算术核对当作科学确认或默认研究策略。

先区分三种问题：已执行候选是否复算一致、固定候选对划分是否敏感，以及包含选择步骤的整个研究策略是否更好。
同一批已用数据换随机种子不会自动变成独立新证据；若在新划分上继续选候选，确认要与这次选择分开。
记录实际参照与改变字段，联合修改可以用于优化，但不能单独归因于某一个因素。
[真实固定配置复算](plans/2026-09-17-research-confirmation.zh-CN.md)中，两个此前单划分获胜配置仅在3/8和5/8的新划分上改善。
这限制了固定配置的迁移结论，没有重新评价完整研究策略，也没有建立默认的复验次数。
