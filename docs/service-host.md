...
本入口已完成隔离 CPU 产品验证。[完整关闭后的显式宿主恢复](service-recovery.md)已接通，
保留同一作用域及累计预算。实际 systemd、GPU、机器重启、旧服务迁移与生产切换
尚未验收；本功能不自动迁移已有全局服务。
...
不要将删除登记目录当作恢复办法。完整关闭后可显式准备新配置并恢复宿主；
非正常崩溃修复、跨版本迁移和真实管理器下的重新安装仍待验收。
