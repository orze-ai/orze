...
卸载先核验三个 unit 的当前配置，再请求注册停止，复查后仅禁用、删除选中的 unit。
服务配置、宿主登记、关闭记录及研究状态保留。已经退出的宿主目前不能通过此入口完成卸载；
不要将删除登记目录当作恢复办法。停止后重新安装、自然策略结束后的管理及冷恢复仍待完成。
...
详细结果见[服务宿主验证记录](plans/2026-09-16-service-host-results.zh-CN.md)。
