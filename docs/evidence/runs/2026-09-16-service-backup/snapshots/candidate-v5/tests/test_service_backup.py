...
def test_late_restored_file_change_cannot_receive_completion(captured, monkeypatch):
    ...
    manifest = json.loads((path/'manifest.json').read_bytes())
    row = next(row for row in manifest['entries'] if row['kind'] == 'file' and row['bytes'] > 0)
    verify = backup._verify
    changed = []
    def change_after_source_recheck(*args, **kwargs):
        result = verify(*args, **kwargs)
        if target.exists() and not changed:
            (target/row['root']/row['path']).write_bytes(b'X' * row['bytes'])
            changed.append(True)
        return result
...
def test_unsupported_wal_policy_refused_before_backup_creation(closed):
    root = closed[0]
    writer = sqlite3.connect(root/'lake.db')
    try:
        assert writer.execute('PRAGMA journal_mode=WAL').fetchone() == ('wal',)
        writer.execute('CREATE TABLE backup_note (value TEXT)')
        writer.execute('INSERT INTO backup_note VALUES (?)', ('committed only in WAL',))
        writer.commit()
        before = (root/'lake.db-wal').read_bytes()
        with pytest.raises(ControllerHOLD, match='controller_database_policy_invalid'):
            create(closed)
        assert not root.with_name(root.name+'-backup').exists()
        assert (root/'lake.db-wal').read_bytes() == before
    finally:
        writer.close()


def test_consistent_image_preserves_current_sql_and_blocks_competing_writer(closed, monkeypatch):
    root = closed[0]
    with sqlite3.connect(root/'lake.db') as writer:
        assert writer.execute('PRAGMA journal_mode').fetchone() == ('delete',)
        writer.execute('CREATE TABLE backup_note (value TEXT)')
        writer.execute('INSERT INTO backup_note VALUES (?)', ('committed source content',))
    stream = backup._stream
    seen = []
    def blocked_writer(*args, **kwargs):
        if not seen:
            other = sqlite3.connect(root/'lake.db', timeout=0)
            try:
    ...
    monkeypatch.setattr(backup, '_stream', blocked_writer)
    report = create(closed)
    path = Path(report['backup'])
    manifest = json.loads((path/'manifest.json').read_bytes())
    row, = (row for row in manifest['entries'] if row['kind'] == 'database')
    with sqlite3.connect(path/'objects'/row['object']) as image:
        assert image.execute('SELECT value FROM backup_note').fetchall() == [('committed source content',)]
    assert seen and dump(path/'objects'/row['object']) == dump(root/'lake.db')
    assert all(not row['path'].endswith(('lake.db-wal','lake.db-shm','lake.db-journal')) for row in manifest['entries'])


def test_actual_public_cli_creates_verifies_and_restores_inactive_data(closed):
    root, _, _, env, service, *_ = closed
    guard = root.with_name(root.name+'-cli-guard')
    guard.mkdir();(guard/'sitecustomize.py').write_text(SITE)
    env = dict(env, PYTHONPATH=str(guard)+os.pathsep+env['PYTHONPATH'])
    target = root.with_name(root.name+'-cli-backup')
    def run(name, args):
        result = subprocess.run([sys.executable,'-m','orze.cli','service','backup',*args],
            cwd=root,env=env,capture_output=True,text=True,timeout=30)
        (guard/(name+'.stdout')).write_text(result.stdout)
        (guard/(name+'.stderr')).write_text(result.stderr)
        assert result.returncode == 0, result.stderr
        return json.loads(result.stdout)
    created = run('create', ['create','--service-config',str(service),'--destination',str(target)])
    args = ['--backup',str(target),'--manifest-sha256',created['manifest_sha256']]
    assert run('verify', ['verify',*args])['activation'] is False
    restored = root.with_name(root.name+'-cli-restored')
    assert run('restore', ['restore',*args,'--destination',str(restored)])['activation'] is False
    manifest = json.loads((target/'manifest.json').read_bytes())
    project = restored/next(row['name'] for row in manifest['roots'] if row['source']==str(root))
    assert dump(project/'lake.db') == dump(root/'lake.db')


def test_destination_parent_redirection_never_copies_source_into_foreign_directory(closed, monkeypatch):
    root = closed[0]
    target = root.with_name(root.name+'-backup')
    retained = root.with_name(root.name+'-retired-backup')
    foreign = root.with_name(root.name+'-foreign')
    foreign.mkdir();(foreign/'keep').write_bytes(b'foreign data')
    publish = backup._Output.publish
    swapped = []
    def swap(output, name, value):
        result = publish(output, name, value)
        if name == 'intent.json' and not swapped:
            output.path.rename(retained)
            output.path.symlink_to(foreign, target_is_directory=True)
            swapped.append(True)
        return result
    monkeypatch.setattr(backup._Output, 'publish', swap)
    with pytest.raises(ControllerHOLD, match='destination_changed'):
        backup.capture(closed[4], target)
    assert swapped and sorted(p.name for p in foreign.iterdir()) == ['keep']
    assert (foreign/'keep').read_bytes() == b'foreign data' and not (retained/'complete.json').exists()


def close_with_external_goal(hosted, *, present):
    root, config, cfg, env, service, start, client, wait, *_ = hosted
    goal = root.with_name(root.name+'-goal.md')
    if present:
        goal.write_text('An external declared project input\n')
    cfg['goal_file'] = str(goal)
    config.write_text(yaml.safe_dump(cfg))
    idea(root, 'idea-0001', 'print("external input backup")')
    process = start()
    wait(lambda: rows(root, 'SELECT state FROM cpu_action_reservations') == [('SETTLED',)])
    assert client('stop')[0] == 0 and process.wait(timeout=5) == 0
    return goal
...
def test_external_declared_input_restores_as_separate_root(hosted_backup):
    goal = close_with_external_goal(hosted_backup, present=True)
    report = create(hosted_backup)
    path = Path(report['backup'])
    manifest = json.loads((path/'manifest.json').read_bytes())
    external, = (row for row in manifest['roots'] if row['source'] == str(goal))
    restored = path.with_name(path.name+'-restored')
    backup.restore(path, report['manifest_sha256'], restored)
    assert (restored/external['name']).read_bytes() == goal.read_bytes()


def test_previously_missing_external_input_appearing_during_copy_blocks_completion(hosted_backup, monkeypatch):
    goal = close_with_external_goal(hosted_backup, present=False)
    stream = backup._stream
    appeared = []
    def create_input(*args, **kwargs):
        result = stream(*args, **kwargs)
        if not appeared:
            goal.write_text('New external input during backup\n')
            appeared.append(True)
        return result
    monkeypatch.setattr(backup, '_stream', create_input)
    with pytest.raises(ControllerHOLD):
        create(hosted_backup)
    path = hosted_backup[0].with_name(hosted_backup[0].name+'-backup')
    assert appeared and not (path/'complete.json').exists()
