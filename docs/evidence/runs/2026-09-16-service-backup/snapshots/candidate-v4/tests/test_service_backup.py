...
def test_wal_image_is_complete_and_other_writers_are_blocked_during_capture(closed, monkeypatch):
    root = closed[0]
    ...
    try:
        ...
        writer.commit()
        assert (root/'lake.db-wal').stat().st_size > 0
        immutable = sqlite3.connect((root/'lake.db').as_uri()+'?immutable=1', uri=True)
        try:
            assert immutable.execute("SELECT name FROM sqlite_master WHERE name='backup_note'").fetchall() == []
        finally:
            immutable.close()
        ...
        image = sqlite3.connect(path/'objects'/row['object'])
        try:
            assert image.execute('SELECT value FROM backup_note').fetchall() == [('committed only in WAL',)]
        finally:
            image.close()
