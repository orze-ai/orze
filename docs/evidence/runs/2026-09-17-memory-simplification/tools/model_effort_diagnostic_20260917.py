...
def main():
    ...
    pilot.credentials()
    root = pilot.ROOT / 'model-effort-20260917' / f'pair-{args.repetition}-{args.arm}'
    root.mkdir(mode=0o700)
    plan = json.loads((pilot.ROOT/'model_effort_plan_20260917.json').read_bytes())
    budget = {'authorized_total_usd': 5000, 'reserved_for_slot_usd': 4,
              'whole_batch_reserved_usd': 8, 'cumulative_reserved_usd': 347,
    ...
    os.environ.update(plan['model_environment'])
    campaign.SETTINGS.update(plan['model_environment'])
    assert hashlib.sha256((pilot.PRO/'src/orze_pro/agents/research_llm.py').read_bytes()).hexdigest() == plan['candidate_backend_sha256']
...
"""
    ...
    pilot.write(root/'plan.json', {'request': request, 'budget': budget,
        ...
        'predeclared_plan_sha256': hashlib.sha256((pilot.ROOT/'model_effort_plan_20260917.json').read_bytes()).hexdigest(),
    ...
    try:
        ...
        post = research_llm._post_json
        def observe_parameters(url, payload, headers, timeout=120):
            parameters = {key: payload.get(key) for key in ('model','max_tokens','output_config')}
            parameters['http_timeout_seconds'] = timeout
            parameters['prompt_sha256'] = hashlib.sha256(payload['messages'][0]['content'].encode()).hexdigest()
            with (root/'request-parameters.jsonl').open('a') as f:
                f.write(json.dumps(parameters,sort_keys=True)+'\n')
            return post(url,payload,headers,timeout=timeout)
        research_llm._post_json = observe_parameters
