...
def main():
    ...
    pilot.credentials()
    root = pilot.ROOT / 'memory-action-pairs-20260917' / f'pair-{args.repetition}-{args.arm}'
    root.mkdir(mode=0o700)
    plan = json.loads((pilot.ROOT/'memory_action_plan_20260917.json').read_bytes())
    budget = {'authorized_total_usd': 5000, 'reserved_for_slot_usd': 2,
              'whole_batch_reserved_usd': 16, 'cumulative_reserved_usd': 307,
              'bound': plan['budget_bound'], 'scope': plan['scope']}
    assert budget['cumulative_reserved_usd'] <= budget['authorized_total_usd']
    baseline = pilot.ROOT/'memory-action-baseline-20260917'
    ...
    prompt_sources = {name: hashlib.sha256((pilot.PRO/'src/orze_pro/agents'/name).read_bytes()).hexdigest()
                      for name in ('memory_consumer.py','memory_updates.py')}
    if args.arm == 'A':
...
'''
    ...
    pilot.write(root/'plan.json', {'request': request, 'budget': budget,
        ...
        'predeclared_plan_sha256': hashlib.sha256((pilot.ROOT/'memory_action_plan_20260917.json').read_bytes()).hexdigest(),
