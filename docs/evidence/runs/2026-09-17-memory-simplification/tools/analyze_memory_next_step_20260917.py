...
BATCH=ROOT/'memory-next-step-pairs-20260917'
...
def summarize(root, process):
    ...
    improved=[e for e in valid if e['verdict']['scheduled_value']>14]
    prompts=[]
    ...
    return {'arm':result['arm'],'repetition':result['repetition'],'error':result['error'],**process,
        ...
        'best_including_seed':max([14]+[e['verdict']['scheduled_value'] for e in valid]),
        'first_improvement_seconds':min((e['observed_seconds'] for e in improved),default=None),
        'first_score_27_seconds':min((e['observed_seconds'] for e in valid if e['verdict']['scheduled_value']==27),default=None),
...
def main():
    ...
    for field in ('data','model','tools','instructions','initial_history','initial_memory','treatment'):
        assert len({json.dumps(p['request']['inputs'][field],sort_keys=True) for p in plans})==1,field
    assert len({p['request']['runtime']['artifact_sha256'] for p in plans})==1
    assert all(p['predeclared_plan_sha256']==sha(ROOT/'memory_next_step_plan_20260917.json') for p in plans)
    for arm in ('A','B'):
        assert len({json.dumps(p['effective_prompt_source_sha256'],sort_keys=True) for p in plans if p['request']['slot']['arm']==arm})==1
    ...
    for row in rows:
        ...
        for p in row['prompts']:
            ...
            if row['arm']=='B':assert p['ending_offers_memory']==(p['updates']['used']<p['updates']['maximum'])
            else:assert p['ending_offers_memory']
    arms=[]
    for arm in ('A','B'):
        group=[r for r in rows if r['arm']==arm]
        counters=Counter(o['reason'] for r in group for o in r['outcomes'])
        arms.append({'arm':arm,'planned':4,'completed':len(group),'projects_with_improvement':sum(r['improved'] for r in group),
            'projects_with_score_27':sum(r['new_best_score']==27 for r in group),
    ...
    for rep in range(4):
        ...
        pairs.append({'repetition':rep,'A':{k:a[k] for k in ('new_best_score','process_wall_seconds','first_improvement_seconds','first_score_27_seconds','usage')},
            'B':{k:b[k] for k in ('new_best_score','process_wall_seconds','first_improvement_seconds','first_score_27_seconds','usage')},
    ...
    previous=read(ROOT/'memory-action-pairs-20260917/summary.json')['cumulative_usage']
    ...
    result={'schema':1,'scope':plan['scope'],'arms':arms,'pairs':pairs,'runs':rows,
        'cumulative_usage':total,'cumulative_reserve_usd':323,'authorized_cap_usd':5000,
        'limitations':['One small authored scheduling task, four paired repetitions, remote sampling uncontrolled.',
            'Both arms are prompted to write a counterexample once; this does not estimate memory versus no-memory benefit.',
