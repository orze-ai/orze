...
out=ROOT/'memory-next-step-check-20260917'
out.mkdir(mode=0o700)
(out/'tmp').mkdir()
names=['src/orze_pro/agents/research.py','tests/test_research_memory_consumer.py','src/orze_pro/agents/memory_consumer.py','src/orze_pro/agents/memory_updates.py','tests/test_research_memory_updates.py']
def hashes():return {n:hashlib.sha256((ROOT/'pro'/n).read_bytes()).hexdigest() for n in names}
before=hashes()
args=['-q','tests/test_research_memory_updates.py','tests/test_research_memory_prompt_admission.py','tests/test_research_memory_api_admission.py','tests/test_research_memory_consumer.py::test_actual_consumer_restores_the_complete_counterexample','tests/test_research_genealogy.py','--tb=short','-p','no:cacheprovider','--basetemp='+str(out/'pytest'),'--junitxml='+str(out/'junit.xml')]
