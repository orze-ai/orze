...
assert (ROOT/'memory-next-step-pairs-20260917/summary.json').is_file()
assert checks['exit_code']==0 and checks['before']==checks['after']
plan=json.loads((ROOT/'optional_memory_workflow_plan_20260917.json').read_bytes())
root=ROOT/'optional-memory-workflow-20260917'
root.mkdir(mode=0o700)
(root/'tmp').mkdir()
(root/'plan.json').write_bytes((ROOT/'optional_memory_workflow_plan_20260917.json').read_bytes())
runner=ROOT/'optional_memory_workflow_20260917.py'
...
print(json.dumps({'planned':4,'completed':len(rows),'successful_processes':sum(r['exit_code']==0 for r in rows)}),flush=True)
