...
def main():
    ...
    pilot.credentials()
    root = pilot.ROOT / 'optional-memory-workflow-20260917' / f'pair-{args.repetition}-{args.arm}'
    root.mkdir(mode=0o700)
    plan = json.loads((pilot.ROOT/'optional_memory_workflow_plan_20260917.json').read_bytes())
    budget = {'authorized_total_usd': 5000, 'reserved_for_slot_usd': 2,
              'whole_batch_reserved_usd': 8, 'cumulative_reserved_usd': 331,
...
"""
    ...
    pilot.write(root/'plan.json', {'request': request, 'budget': budget,
        ...
        'predeclared_plan_sha256': hashlib.sha256((pilot.ROOT/'optional_memory_workflow_plan_20260917.json').read_bytes()).hexdigest(),
