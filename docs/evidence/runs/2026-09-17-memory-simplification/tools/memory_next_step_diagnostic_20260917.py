...
Only A restores the frozen baseline prompt functions; no generated output is changed.
"""
import argparse
import ast
import hashlib
...
def main():
    ...
    pilot.credentials()
    root = pilot.ROOT / 'memory-next-step-pairs-20260917' / f'pair-{args.repetition}-{args.arm}'
    root.mkdir(mode=0o700)
    plan = json.loads((pilot.ROOT/'memory_next_step_plan_20260917.json').read_bytes())
    budget = {'authorized_total_usd': 5000, 'reserved_for_slot_usd': 2,
              'whole_batch_reserved_usd': 16, 'cumulative_reserved_usd': 323,
              'bound': plan['budget_bound'], 'scope': plan['scope']}
    assert budget['cumulative_reserved_usd'] <= budget['authorized_total_usd']
    baseline = pilot.ROOT/'memory-next-step-baseline-20260917'
    from orze_pro.agents import memory_consumer, memory_updates
    ...
    if args.arm == 'A':
        for name, key in [('memory_consumer.py','baseline_prompt_sha256'),
                          ('memory_updates.py','baseline_update_instructions_sha256')]:
            assert hashlib.sha256((baseline/name).read_bytes()).hexdigest() == plan[key]
        namespace = {}
        exec(compile((baseline/'memory_consumer.py').read_text(), str(baseline/'memory_consumer.py'), 'exec'), namespace)
        memory_consumer.prompt_text = namespace['prompt_text']
        tree = ast.parse((baseline/'memory_updates.py').read_text())
        node = next(n for n in tree.body if isinstance(n, ast.Assign)
                    and any(isinstance(t,ast.Name) and t.id=='INSTRUCTIONS' for t in n.targets))
        memory_updates.INSTRUCTIONS = ast.literal_eval(node.value)
        prompt_sources = {name: hashlib.sha256((baseline/name).read_bytes()).hexdigest()
                          for name in ('memory_consumer.py','memory_updates.py','research.py')}
    ...
    inputs['initial_history'] = {'valid': plan['seed_valid'], 'invalid': plan['seed_invalid']}
...
'''
    ...
    pilot.write(root/'plan.json', {'request': request, 'budget': budget,
        ...
        'predeclared_plan_sha256': hashlib.sha256((pilot.ROOT/'memory_next_step_plan_20260917.json').read_bytes()).hexdigest(),
    ...
    try:
        ...
        seeds = {
            'valid': plan['seed_valid'],
            'overload': plan['seed_invalid'],
        }
        ...
        if args.arm == 'A':
            frozen = baseline/'research.py'
            assert hashlib.sha256(frozen.read_bytes()).hexdigest() == plan['baseline_research_sha256']
            tree = ast.parse(frozen.read_text())
            node = next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='build_prompt')
            namespace = dict(research.__dict__)
            exec(compile(ast.Module(body=[node],type_ignores=[]),str(frozen),'exec'),namespace)
            research.build_prompt = namespace['build_prompt']
