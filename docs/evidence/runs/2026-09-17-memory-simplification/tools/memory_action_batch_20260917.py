...
checks=json.loads((ROOT/'memory-action-check-20260917/result.json').read_bytes())
assert checks['exit_code']==0 and checks['before']==checks['after']
plan=json.loads((ROOT/'memory_action_plan_20260917.json').read_bytes())
root=ROOT/'memory-action-pairs-20260917'
root.mkdir(mode=0o700)
(root/'tmp').mkdir()
(root/'plan.json').write_bytes((ROOT/'memory_action_plan_20260917.json').read_bytes())
runner=ROOT/'memory_action_diagnostic_20260917.py'
