...
BATCH=ROOT/'memory-action-pairs-20260917'
...
def main():
    ...
    for row in rows:
        ...
        for p in row['prompts']:
            ...
            else:assert p['write_template_offered']
    arms=[]
    for arm in ('A','B'):
    ...
    for rep in range(4):
        ...
        pairs.append({'repetition':rep,'A':{k:a[k] for k in ('new_best_score','process_wall_seconds','first_improvement_seconds','usage')},
            'B':{k:b[k] for k in ('new_best_score','process_wall_seconds','first_improvement_seconds','usage')},
    ...
    previous=read(ROOT/'real-memory-pilot-20260916-feedback/summary.json')['cumulative_usage']
    ...
    result={'schema':1,'scope':plan['scope'],'arms':arms,'pairs':pairs,'runs':rows,
        'cumulative_usage':total,'cumulative_reserve_usd':307,'authorized_cap_usd':5000,
