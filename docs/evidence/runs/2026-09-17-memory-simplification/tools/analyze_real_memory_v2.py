...
sys.path.insert(0, str(ROOT / 'orze'))
...
def summarize_run(directory, measured):
    ...
    for observation in observations:
        ...
        assert scheduling.evaluate(record['request']['inputs']['data'], raw.encode(), evaluated['protocol']) == evaluated['verdict'], 'candidate verdict mismatch'
        try:
...
def main(publish):
    result = {'scope': 'Two small authored scheduling tasks; exploratory real-model pilot, not cross-domain or long-memory validation.',
              ...
              'total_budget_usd': 5000, 'conservative_reserved_upper_bound_usd': 181, 'models': {}}
    ...
    if publish:
        with (CURRENT/'independent-summary-v2.json').open('x') as stream:
