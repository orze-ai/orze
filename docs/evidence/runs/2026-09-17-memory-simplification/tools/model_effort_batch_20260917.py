...
checks=json.loads((ROOT/'model-settings-check-20260917-v2/result.json').read_bytes())
assert (ROOT/'model-capacity-20260917/summary.json').is_file()
assert checks['exit_code']==0 and checks['before']==checks['after']
plan=json.loads((ROOT/'model_effort_plan_20260917.json').read_bytes())
root=ROOT/'model-effort-20260917'
root.mkdir(mode=0o700)
(root/'tmp').mkdir()
(root/'plan.json').write_bytes((ROOT/'model_effort_plan_20260917.json').read_bytes())
runner=ROOT/'model_effort_diagnostic_20260917.py'
