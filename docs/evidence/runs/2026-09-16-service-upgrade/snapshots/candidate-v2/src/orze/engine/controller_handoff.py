...
def _profile(cfg):
    from orze.core.controller_profile import handoff_profile, profile_fingerprint
    if not handoff_profile(cfg):
...
def _inherited_channel(environment_key=_FD_ENV):
    ...
    channel.set_inheritable(False)
    channel.setsockopt(socket.SOL_SOCKET, socket.SO_PASSCRED, 1)
    os.environ.pop(environment_key, None)  # Never leak bootstrap admission into workers.
    return channel
...
def _history_observer(route, identity_json, binding_json):
    ...
    This cannot issue a stop or grant. Live handoffs also require their own
    kernel observer; explicit closed-service recovery separately checks pinned
    closure, process absence and a new instruction before issuing a new grant.
    This object itself only checks sealed history and ancestry.
    """
    import re
    from orze.core.controller_profile import controller_profile, _physical_gpus
    from orze.core.cpu_execution import cpu_execution, cpu_handoff_profile
    ...
    observer._cpu_declaration = cpu_execution(route.cfg) if cpu_handoff_profile(route.cfg) else None
    ...
    expected_binding = {"schema": 1, "controller_id": observer.controller_id, "identity": identity,
        "profile": controller_profile(route.cfg), "config_sha256": route.fingerprint,
        "physical_gpus": _physical_gpus(route.cfg),
...
def _closed_history_catalog(conn, *, cpu=False):
    ...
    if (unknown("ideas", "status", STATUS_TO_STATE)
            ...
            or unknown("idea_stage_state", "stage", ("action",) if cpu else PIPELINE_STAGES)
    ...
    if cpu and conn.execute("SELECT 1 FROM main.ideas WHERE kind!='native_cpu_action' "
                            "OR kind IS NULL LIMIT 1").fetchone():
        raise ControllerHOLD("controller_cpu_handoff_task_kind_changed")
...
def _validate_history(route, source_id, generation, *, expected=None, closed_inventory=True,
                      connection=None):
    """Read closed history, optionally in the caller's exact SQLite view.

    A supplied connection is borrowed without writes, commit or close. This
    lets successor consumption revalidate history inside its existing writer
    transaction; a separate read connection cannot see that writer's changes.
    """
    ...
    def history_connection():
        ...
        if (not isinstance(connection, sqlite3.Connection)
                or _route(connection) != (route.db, route.db_witness)):
            raise ControllerHOLD("controller_handoff_history_connection_changed")
        try:
            yield connection
        finally:
            route.check()
            if _route(connection) != (route.db, route.db_witness):
                raise ControllerHOLD("controller_handoff_history_connection_changed")
    ...
    with history_connection() as conn:
        ...
        if closed_inventory:
            _closed_history_catalog(conn, cpu=cpu_handoff_profile(route.cfg))
        member_schema(conn)
    ...
    for index, (registration, session) in enumerate(snapshots):
        ...
        observer.verify_drain(ack, conn=connection)
        current, actual_session = observer.check(connection)
    ...
    with history_connection() as conn:
        ...
        from orze.core.cpu_execution import cpu_execution, cpu_handoff_profile
        if closed_inventory and cpu_handoff_profile(route.cfg):
            from orze.engine.controller_cpu import budget_history_proof
            budget_history_proof(conn, route.scope, cpu_execution(route.cfg), source_id)
...
class _Admission:
    ...
    def consume(self, conn, ctx):
        ...
        # Use the registration writer's actual view, including its uncommitted
        # rows. A separate read-only connection would miss same-TX corruption.
        _validate_history(self._route, self.source_controller_id, self.generation - 1,
                          expected=self._payload["history_sha256"], connection=conn)
    ...
    def _check_resources(self, session):
        """Caller holds the context and GPU registry locks, including at COMMIT."""
        from orze.core.cpu_execution import cpu_handoff_profile
        if cpu_handoff_profile(session.orze.cfg):
            from orze.engine.controller_cpu import check_handoff_resources
            return check_handoff_resources(session)
        from orze.core.gpu_lease import GpuLeaseSet, _registered
        from orze.engine.controller_session import _file_witness
        session._validate_runtime()
        owned, witnesses = session._gpu, session._gpu_witness
        if (type(owned) is not GpuLeaseSet or owned is not session.orze._gpu_leases or owned._closed
    ...
    def mark_started(self, session):
        ...
        from orze.core.gpu_lease import _registry_lock
        self._check_route()
        ...
        with session.ctx.guard(), _registry_lock:
            ...
            witnesses = self._check_resources(session)
            ...
            from orze.core.cpu_execution import cpu_handoff_profile
            if cpu_handoff_profile(session.orze.cfg):
                started.update(witnesses)
            else:
                started.update(gpu_scope=sorted(session.orze.gpu_ids), lease_count=len(witnesses),
        ...
        if _encode(packet) != _encode({"schema": 1, "event": "COMMIT", "grant_id": self.grant_id,
                                      "started_sha256": _sha(encoded.encode())}):
            raise ControllerHOLD("controller_handoff_commit_packet_invalid")
        with session.ctx.guard(), _registry_lock:
            session.ctx.check_admission()
            self._check_route()
            current_resources = self._check_resources(session)
            if cpu_handoff_profile(session.orze.cfg) and current_resources != witnesses:
                raise ControllerHOLD("controller_cpu_handoff_resources_changed")
            with self._route.connection(write=True) as conn:
                if self._check_row(conn, {"PREPARED"})[8] != encoded:
...
def _verify_started(route, row, *, state, expected_process=None, children=None, deadline=None):
    """Fresh current registration + resource-start record, not an ACK label."""
    from orze.engine.controller_session import _Observer, _closed_observed
    from orze.core.cpu_execution import cpu_execution, cpu_handoff_profile
    payload = _validate_payload(route, row)
    ...
    allow_completion = children is not None and state == 'STARTED'
    observer = _Observer(route.cfg, _children=children) if allow_completion else _Observer(route.cfg)
    try:
        ...
        if cpu_handoff_profile(route.cfg):
            from orze.engine.controller_cpu import scope_readiness
            with route.connection() as conn:
                conn.execute('BEGIN')
                expected.update(scope_readiness(conn, route.scope, cpu_execution(route.cfg)))
        else:
            pid_hash = started.get("pid_file_sha256")
            if (type(pid_hash) is not str or len(pid_hash) != 64
                    or any(c not in "0123456789abcdef" for c in pid_hash)):
                raise ControllerHOLD("controller_handoff_start_binding_changed")
            expected.update(gpu_scope=sorted(route.cfg["gpu_scheduling"]["allowed_gpus"]),
                lease_count=len(route.cfg["gpu_scheduling"]["allowed_gpus"]),
                pid_file_sha256=pid_hash, stage="leased_before_first_probe")
        if (observer.controller_id != payload["target_controller_id"]
                ...
                or _encode(started) != _encode(expected)):
            raise ControllerHOLD("controller_handoff_start_binding_changed")
        complete = False
        if registration[1:] != ("ACTIVE", None, None) or session[2:] != (None, None) or observer.exited():
            if not allow_completion:
                raise ControllerHOLD("controller_handoff_start_binding_changed")
            # STARTED is already durable. The actual owned successor may have
            # completed policy work before its reply was delivered. Observe
            # its existing drain; do not issue Stop or grant a second launch.
            limit = time.monotonic() + 60 if deadline is None else deadline
            while not observer.exited():
                children.admit()
                observer.check()
                if time.monotonic() >= limit:
                    raise ControllerHOLD('controller_handoff_completion_unconfirmed')
                select.select([observer.pidfd], [], [], min(.05, max(0, limit - time.monotonic())))
            _closed_observed(observer)
            _validate_history(route, payload['target_controller_id'], payload['target_generation'])
            complete = True
        with route.connection() as conn:
...
class _ChildSet:
    """Retain actual direct children for a persistent external service owner.

    This records already created children, never grants a launch or adopts a
    PID from storage. The normal handoff handshake remains the sole admission.
    """
    def __init__(self):
    ...
    def check(self):
        if self.pid != os.getpid() or self.thread != threading.get_ident():
            raise ControllerHOLD("controller_handoff_child_owner_changed")

    def retain(self, child, process, pidfd):
        # Capture an already created child even if a stop just arrived. This
        # grants no GO/COMMIT; admitting more work uses admit() separately.
        self.check()
        if (len(self.children) >= 65 or child.pid != process["pid"]
                or process_identity(child.pid) != (process, self.pid)
                or child.poll() is not None
                or select.select([pidfd], [], [], 0)[0]):
            raise ControllerHOLD("controller_handoff_child_retention_unconfirmed")
        self.children.append((child, dict(process), os.dup(pidfd)))

    def admit(self):
        self.check()
        if self.quiescing:
            raise ControllerHOLD("controller_handoff_child_owner_quiescing")

    def owns(self, process):
        self.check()
        return any(identity == process and child.poll() is None
                   and not select.select([fd], [], [], 0)[0]
                   for child, identity, fd in self.children)

    def capture(self, process):
        """Duplicate our original child handle, including after its exit.

        A process identity from storage alone cannot enter this collection.
        Never reopen a dead child's numeric PID; it may already be reused.
        """
        self.check()
        for child, identity, fd in self.children:
            if identity != process:
            ...
            if not select.select([fd], [], [], 0)[0]:
                try:
                    ...
                    if current != (identity, self.pid):
                        raise ControllerHOLD("controller_handoff_child_identity_changed")
            return os.dup(fd)
        raise ControllerHOLD("controller_handoff_child_not_owned")

    def live(self):
        self.check()
        return sum(child.poll() is None for child, _, _ in self.children)

    def close(self):
        self.check()
        for child, _, fd in self.children:
            child.poll()  # Reap our own completed children only.
            os.close(fd)
        self.children.clear()


class _Coordinator:
    def __init__(self, cfg, request_id, timeout, children=None):
        if children is not None:
            if type(children) is not _ChildSet:
                raise ControllerHOLD("controller_handoff_child_owner_invalid")
            children.admit()
    ...
    def check(self, *, allow_completed=False):
        ...
        if (self.child_fd is not None and select.select([self.child_fd], [], [], 0)[0]
                and not (allow_completed and self.children is not None)):
            raise ControllerHOLD("controller_handoff_successor_exited")
    ...
    def run(self):
        ...
        if previous is not None:
            ...
            return _verify_started(self.route, previous, state="STARTED", children=self.children, deadline=self.deadline)
        self.prepare_source()
        self.check()
        ...
        self.child = subprocess.Popen([sys.executable, "-m", "orze.cli", "-c", str(self.route.config_file)],
            ...
            stdin=subprocess.DEVNULL, stdout=None if self.children is not None else subprocess.DEVNULL,
            stderr=None if self.children is not None else subprocess.DEVNULL, start_new_session=True)
        process = process_identity(self.child.pid)[0]
        self.child_fd = _capture(process)
        if self.children is not None:
            self.children.retain(self.child, process, self.child_fd)
...
def restart_controller(cfg, request_id, timeout=60, *, _children=None):
    ...
    return _run_coordinator(_Coordinator(cfg, request_id, timeout, _children))


def _run_coordinator(coordinator):
    """Keep issuer lifetime handling shared with explicit closed-service recovery."""
    from orze.engine.controller_control import current_controller
    if current_controller() is not None:
        raise ControllerHOLD("controller_handoff_external_issuer_required")
