...
from orze.engine.controller_session import _Observer, _file_witness, _stop_observed, _closed_observed
...
def _service(path):
    ...
    if (type(declaration) is not dict or declaration != PROFILE
            ...
            or value.get("method") not in {"process", "systemd"}):
...
class Host:
    ...
    def _check_inputs(self):
        ...
        contract = audit_runtime_contract(self.svc, expected_host_pid=(
            os.getpid() if self.svc["method"] == "systemd" else None))
    ...
    def _observe(self):
        self._check_owner()
        observer = _Observer(self.cfg, _children=self.children)
        if self.svc["method"] == "systemd" and not observer.exited():
            from orze.service.runtime_contract import manager_cgroup
            try:
                matches = manager_cgroup(observer.identity["process"]["pid"]) == manager_cgroup(os.getpid())
            except OSError:
                matches = observer.exited()
            if not matches:
                observer.close()
                raise ControllerHOLD("service_host_controller_cgroup_changed")
        return observer

    def _completed(self, observer):
        from orze.service.closed_state import verify_controller_record
        proof = {"kind": "stopped", **asdict(_closed_observed(observer))}
        verify_controller_record(self.cfg, proof)
    ...
    def start(self):
        ...
        if 'recovery' in self.svc:
            ...
            if observed['controller']['controller_id'] != result.target_controller_id:
                raise ControllerHOLD('service_host_recovered_successor_not_owned')
            self.started_at = time.monotonic()
            _create(self.lease.lock_dir / 'ready.json', {'schema': 1, 'controller_id': result.target_controller_id})
            return
        ...
        try:
            ...
            self._check_owner()
            self._check_inputs()
            if self.svc["method"] == "systemd":
                from orze.service.runtime_contract import manager_cgroup
                if manager_cgroup(child.pid) != manager_cgroup(os.getpid()):
                    raise ControllerHOLD("service_host_child_cgroup_changed")
    ...
    def status(self):
        ...
        try:
            observer.check()
            result = {"host_process": self.registration["process"], "live_children": self.children.live(),
                    "state": "running",
                    "controller": {"controller_id": observer.controller_id,
                                   "process": observer.identity["process"]}}
            if observer.exited():
                result.update(state="stopped", closure=self._completed(observer), live_children=0)
            return result
        finally:
            observer.close()
    ...
    def restart(self, request_id, source, timeout):
        ...
        was_pending = self.pending_handoff is not None
        ...
        self._check_inputs()
        status = self.status()
        current = status["controller"]["controller_id"]
        route = _Route(self.cfg)
        with route.connection() as conn:
            previous = _optional_request(conn, request_id)
        if status['state'] != 'running' and previous is None:
            raise ControllerHOLD('service_host_controller_completed')
        ...
        if previous is None or was_pending:
            self.started_at = time.monotonic()
        self.pending_handoff = None
        return {"kind": "handoff", "controller_state": observed['state'], **asdict(result)}
    ...
    def stop(self, timeout):
        ...
        self.children.quiescing = True
        observer = self._observe()
        try:
            if not observer.exited():
                try:
                    _stop_observed(observer, timeout)
                except ControllerHOLD as exc:
                    if str(exc) != "controller_exited_before_stop_request" or not observer.exited():
                        raise
            proof = self._completed(observer)
        finally:
            observer.close()
        if self.children.live():
            raise ControllerHOLD("service_host_children_not_closed")
