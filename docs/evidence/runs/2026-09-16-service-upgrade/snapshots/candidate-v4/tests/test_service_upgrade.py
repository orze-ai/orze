...
def test_unstarted_upgrade_can_be_superseded_by_old_runtime_recovery(runtimes):
    hosted, destination, frozen = runtimes
    root, config, cfg, env, service, start, client, wait, prior, captures, births, before_birth = hosted
    target, archive, original_env = _prepare_upgrade(runtimes)
    old_target = root/'remain-on-old-runtime.json'
    prepared = prepare(root, original_env, service, old_target, 'remain-on-old-runtime')
    assert prepared.returncode == 0, prepared.stderr
    env.clear(); env.update(original_env)
    idea(root, 'idea-0002', 'print("old runtime retained before upgrade admission")')
    old = start(service_config=old_target)
    wait(lambda: rows(root, 'SELECT state FROM cpu_action_reservations') == [('SETTLED',), ('SETTLED',)])
    assert client('stop')[0] == 0 and old.wait(timeout=5) == 0
    prior[:] = captures
    env.clear(); env.update(destination)
    stale = start(wait_ready=False, service_config=target)
    assert stale.wait(timeout=15) == 75
    assert len(births) == 2 and rows(root, 'SELECT request_id,state FROM controller_handoffs') == [('remain-on-old-runtime', 'STARTED')]
    assert all(select.select([fd], [], [], 0)[0] for fd in captures)
    assert target.exists() and archive.joinpath('complete.json').exists()
    (root/'backout-result.json').write_text(json.dumps({'old_recovery': str(old_target), 'stale_upgrade_exit': 75,
        'births': len(births), 'reservations': rows(root, 'SELECT * FROM cpu_action_reservations')}, sort_keys=True, indent=2))


def test_interrupted_upgrade_keeps_pending_grant_and_denies_relaunch(runtimes):
    hosted, destination, frozen = runtimes
    root, config, cfg, env, service, start, client, wait, prior, captures, births, before_birth = hosted
    target, archive, original_env = _prepare_upgrade(runtimes)
    latch = root/'results/.orze_disabled'
    before_birth.append(latch.touch)
    rejected = start(wait_ready=False, service_config=target)
    wait(lambda: rejected.poll() is not None)
    assert rejected.returncode == 75 and len(births) == 2
    assert rows(root, 'SELECT generation FROM controller_instances') == [(0,)]
    assert rows(root, 'SELECT state FROM controller_handoffs') == [('SPAWNING',)]
    assert rows(root, 'SELECT COUNT(*) FROM execution_attempts') == [(1,)]
    # Removing this fixture-owned stop request does not erase the durable intent.
    latch.unlink()
    old_target = root/'cannot-recover-unknown-upgrade.json'
    old = prepare(root, original_env, service, old_target, 'cannot-recover-unknown')
    assert old.returncode == 75 and not old_target.exists()
    retry = start(wait_ready=False, service_config=target)
    assert retry.wait(timeout=15) == 75 and len(births) == 2
    assert all(select.select([fd], [], [], 0)[0] for fd in captures)
    (root/'pending-result.json').write_text(json.dumps({'returncodes': [rejected.returncode, old.returncode, retry.returncode],
        'grant': rows(root, 'SELECT request_id,state FROM controller_handoffs'), 'births': len(births),
        'heads': rows(root, 'SELECT * FROM controller_scope_heads')}, sort_keys=True, indent=2))
