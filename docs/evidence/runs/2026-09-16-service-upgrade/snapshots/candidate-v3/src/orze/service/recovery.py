...
from orze.engine.controller_handoff import (
    _Coordinator, _Route, _history_observer, _process, _run_coordinator,
)
...
def _inactive(svc):
    ...
    if all(p.get('Id') == unit and p.get('LoadState') == 'not-found'
           ...
           and p.get('UnitFileState') in {'', 'disabled', 'static'}
           and (unit == units[1] or p.get('MainPID') == '0' and p.get('ControlGroup') == '')
           for unit, p in zip(units, properties)):
        return
    if not all(p['LoadState'] == 'loaded' and p['UnitFileState'] in (
               {'disabled', 'static'} if unit == units[2] else {'disabled'})
               for unit, p in zip(units, properties)):
