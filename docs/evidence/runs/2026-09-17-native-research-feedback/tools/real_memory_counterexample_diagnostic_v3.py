...
def main():
    pilot.credentials()
    root = pilot.ROOT / 'real-memory-counterexample-diagnostic-20260916-v3'
    root.mkdir(mode=0o700)
    budget = {'authorized_total_usd': 5000, 'prior_reserved_upper_bound_usd': 249,
              'additional_upper_bound_usd': 4, 'total_reserved_upper_bound_usd': 253,
              'bound': 'Two research cycles, each conservatively 100000 tokens at $10/M; reserve $4. No fallback.',
    ...
    inputs['model'] = dict(pilot.MODELS['sonnet5'])
    inputs['instructions'] += '''
    ...
    try:
        ...
        for cycle in (1,2):
            ...
            research.run_research_cycle('anthropic', cycle, root/'ideas.md', root/'results', cfg['report'],
                num_ideas=2, model='claude-sonnet-5', endpoint='https://api.anthropic.com/v1',
