...
def _systemd_properties(unit="orze.service") -> dict:
    keys = (
        "Restart", "WorkingDirectory", "ExecStart", "ExecStartPre",
        "Environment", "EnvironmentFiles", "PassEnvironment",
        "UnsetEnvironment", "ActiveState", "UnitFileState", "Type", "KillMode", "MainPID", "ControlGroup",
    )
...
def _unit_errors(svc_cfg: dict, properties: dict) -> list[str]:
    ...
    if "service_owner" in svc_cfg:
        required_start = (str(svc_cfg.get("python", "")), "-m", "orze.service.host",
                          "--service-config", str(svc_cfg.get("service_config_file", "")))
        if properties.get("Type") != "simple" or properties.get("KillMode") != "control-group":
            errors.append("systemd_service_host_lifetime_drift")
    ...
    if "service_owner" in svc_cfg:
        required_pre += ("--service-config", str(svc_cfg.get("service_config_file", "")))
    if not exec_pre:
    ...
    *,
    ...
    if svc_cfg.get("method") == "systemd":
        try:
            ...
            elif "service_owner" in svc_cfg:
                from orze.service.scoped import unit_names
                effective = _systemd_properties(unit_names(svc_cfg)[0])
            else:
                effective = _systemd_properties()
            errors.extend(_unit_errors(svc_cfg, effective))
            if expected_host_pid is not None:
                if ("service_owner" not in svc_cfg or type(expected_host_pid) is not int
                        or expected_host_pid <= 0 or effective.get("MainPID") != str(expected_host_pid)):
                    errors.append("systemd_service_host_pid_mismatch")
                try:
                    if manager_cgroup(expected_host_pid) != effective.get("ControlGroup"):
                        errors.append("systemd_service_host_cgroup_mismatch")
                except (OSError, ValueError):
                    errors.append("systemd_service_host_cgroup_unavailable")
...
def manager_cgroup(pid):
    """Read the manager hierarchy for a captured process; no PID authority."""
    if type(pid) is not int or pid <= 0:
        raise ValueError("service_host_pid_invalid")
    rows = [line.split(":", 2) for line in Path(f"/proc/{pid}/cgroup").read_text().splitlines()]
    groups = [row[2] for row in rows if len(row) == 3 and (row[1] == "" or "name=systemd" in row[1].split(","))]
    if len(groups) != 1 or not groups[0].startswith("/"):
        raise ValueError("service_host_cgroup_unavailable")
    return groups[0]
...
def main(argv=None) -> int:
    ...
    service_config = None
    if "--service-config" in argv:
        index = argv.index("--service-config")
        if index + 1 >= len(argv):
            return 2
        service_config = argv[index + 1]
    ...
    if service_config is None:
        ...
        from orze.service.scoped import selected
        svc_cfg = selected(service_config)
    if not svc_cfg:
        report = {
            "schema_version": 1,
            "contract_ok": False,
