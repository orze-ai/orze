...
def check_and_restart(svc_cfg):
    ...
    def _check_runtime_and_process():
        ...
        if "service_owner" in svc_cfg:
            from orze.service.host import PROFILE, request
            if (svc_cfg["service_owner"] != PROFILE
                    or type(svc_cfg["service_owner"].get("version")) is not int):
                raise WatchdogLaunchError("service_host_declaration_invalid")
            try:
                outcome = request(svc_cfg["service_config_file"], "watchdog")
            except Exception as exc:
                raise WatchdogLaunchError("service_host_unconfirmed", (type(exc).__name__,)) from None
            _resolve_failure_loop(results_dir, hostname, "restart_succeeded" if outcome.get("kind") == "handoff" else "service_healthy")
            _log("Registered service host checked")
            return
...
def main(argv=None):
    ...
    svc_cfg = load_service_config(args.service_config)
    if not svc_cfg:
    ...
    try:
        ...
        sys.exit(1)
    if "service_owner" not in svc_cfg:
