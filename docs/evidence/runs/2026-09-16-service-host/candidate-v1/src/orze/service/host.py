...
from orze.engine.controller_control import ControllerHOLD, _path
...
def request(service_config, operation, *, request_id=None, source_controller_id=None, timeout=60):
    ...
    owner_file = scope / "_service_host.lock/owner.json"
...
class Host:
    ...
    def start(self):
        self._check_inputs()
        self.lease = _acquire(self.scope / "_service_host.lock")
        ...
        child = subprocess.Popen([sys.executable, "-m", "orze.cli", "-c", str(self.config_file)],
                                 cwd=self.svc["workdir"], env=self.environment, close_fds=True,
