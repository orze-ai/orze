...
def _save_service_config(config_file, results_dir, workdir, python,
                         method, stall_threshold, log_file, service_config_file=None):
    ...
    if service_config_file is not None:
        from orze.service.host import PROFILE, _create
        destination = Path(service_config_file).absolute()
        data.update(service_owner=PROFILE, service_config_file=str(destination))
...
def _install_systemd(svc_cfg):
    ...
    if "service_owner" in svc_cfg:
        ...
        return install_units(svc_cfg, _SYSTEMD_DIR)
...
def install(config_file, method="auto", stall_threshold=1800, service_config_file=None):
    ...
    config_file = str(Path(config_file).resolve())
    if service_config_file is not None:
        service_config_file = str(Path(service_config_file).absolute())
        ...
        try:
            ...
            if not handoff_profile(cfg):
                raise RuntimeError("service_host_handoff_profile_required")
        finally:
            os.chdir(original_cwd)
    else:
        cfg = load_project_config(config_file)
        if cfg.get("controller_control") is not None:
            raise RuntimeError("registered_service_requires_explicit_service_config")
    ...
    if service_config_file is not None and method != "systemd":
        raise RuntimeError("service_host_install_requires_systemd")

    # Save service config
    arguments = (config_file, results_dir, workdir, python, method, stall_threshold, log_file)
    svc_cfg = (_save_service_config(*arguments) if service_config_file is None
               else _save_service_config(*arguments, service_config_file=service_config_file))
    ...
    print(f"\n  Service config: {service_config_file or SERVICE_CONFIG_PATH}")
...
def uninstall(service_config_file=None):
    """Uninstall the orze watchdog service (tries both methods for safety)."""
    print(f"\n\033[1mOrze — Service Uninstall\033[0m")
    if service_config_file is not None:
        from orze.service.scoped import selected, uninstall_units
        uninstall_units(selected(service_config_file), _SYSTEMD_DIR)
        print("  Selected service removed; configuration and research state retained.")
        return
