...
def properties(svc):
    ...
    return {'Restart': 'no', 'Type': 'simple', 'KillMode': 'control-group', 'MainPID': '12000',
...
def test_two_installs_have_distinct_owner_timer_and_configuration(projects, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runtime_contract, '_systemd_properties', lambda unit: properties(next(x for x in projects if scoped.unit_names(x)[0] == unit)))
    for svc in projects:
...
def test_effective_unit_and_owner_drift_are_rejected(projects, monkeypatch, mutation):
    ...
    assert runtime_contract.audit_runtime_contract(svc, properties=props, expected_host_pid=12000)['startup_allowed']
    ...
    assert not runtime_contract.audit_runtime_contract(svc, properties=props, expected_host_pid=12000)['startup_allowed']
...
def test_partial_enable_closes_selected_host_before_disabling(projects, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runtime_contract, 'audit_runtime_contract', lambda value: {'startup_allowed': True})
...
def test_status_logs_audit_and_uninstall_select_the_same_project(projects, tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(runtime_contract, '_systemd_properties', lambda unit: observed.append(('audit', unit)) or properties(projects[0]))
