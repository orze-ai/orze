...
from orze.engine.supervisor_worker import (
    RUNTIME_LEASE_PROTOCOL, RuntimeLeaseExpired, capture_runtime_lease,
    runtime_lease_now,
)
...
def _owned(owner, lake, cfg, *, states=("RUNNING",)):
    from orze.core.cpu_execution import runtime_lease_config
    handle, folder = owner.handle, owner.folder
    if not same(runtime_lease_config(cfg), owner.lease_config):
        raise CPUActionHOLD("cpu_runtime_lease_configuration_changed")
...
def _lease_observation(owner):
    """Separate current execution authority from retained stop authority."""
    descriptor = owner.binding["runtime_lease"]
    observed = runtime_lease_now(descriptor)
    expired = observed >= descriptor["deadline_ns"]
    if owner.lease_expired and not expired:
        raise CPUActionHOLD("cpu_runtime_lease_clock_reversed")
    owner.lease_expired = owner.lease_expired or expired
    return {"schema": 1, "status": "expired" if owner.lease_expired else "authorized",
            "observed_ns": observed}


def _require_active(owner):
    observed = _lease_observation(owner)
    if observed["status"] == "expired":
        raise RuntimeLeaseExpired(observed["observed_ns"])
    return observed
...
def launch(idea_id, results_dir, cfg, *, lake, action, permit, admission, domain_run=None):
    ...
    from orze.core.cpu_execution import runtime_lease_config, runtime_lease_seconds
    owner = None
    try:
        AttemptRef(idea_id, "action", "validation", 1)
        action = validate_action(action)
        lease_config = runtime_lease_config(cfg)
        lease_seconds = runtime_lease_seconds(cfg, action["timeout_seconds"])
        ...
        if not callable(admission):
            raise CPUActionHOLD("cpu_action_admission_required")
        admission()
        if not same(runtime_lease_config(cfg), lease_config):
            raise CPUActionHOLD("cpu_runtime_lease_configuration_changed")
        ...
        binding = {"origin": "native_cpu_action", "kind": "native_cpu_action", "resource": "cpu",
            ...
            "process_supervision_protocol": RUNTIME_LEASE_PROTOCOL,
        ...
        with execution_transaction(lake, folder) as tx:
            ...
            binding["runtime_lease"] = capture_runtime_lease(lease_seconds)
            ...
            tx.watch_cpu_runtime_lease(ref, binding["runtime_lease"])
        ...
        with ExitStack() as streams:
            ...
            process = prepare_supervised(list(action["command"]),
                ...
                runtime_lease=copy.deepcopy(owner.binding["runtime_lease"]))
            ...
            if (ready["command_sha256"] != binding["command_sha256"]
                    or ready["protocol"] != RUNTIME_LEASE_PROTOCOL
                    or not same(ready.get("runtime_lease"), owner.binding["runtime_lease"])):
            ...
            with execution_transaction(lake, folder) as tx:
                ...
                running = {**owner.binding, "supervision": ready, "process_pid": process.pid,
            ...
            # Compatibility diagnostic only; durable CLOCK_BOOTTIME is the
            # sole runtime/publication deadline and includes preparation.
            remaining = (binding["runtime_lease"]["deadline_ns"]
                         - runtime_lease_now(binding["runtime_lease"])) / 1e9
            owner.deadline = owner.started_at + max(0.0, remaining)
            try:
                _require_active(owner)
                process.start()
            except RuntimeLeaseExpired:
                owner.lease_expired = True
                # RUNNING is already bound, but GO was not authorized. The
                # same supervisor closes its unopened gate autonomously;
                # harvest can settle only the actual closed tree.
                pass
        return handle
...
def harvest(handle, results_dir, cfg, *, lake, permit):
    ...
    try:
        ...
        lease_evidence = _lease_observation(owner)
        ret = owner.process.poll()
        if ret is None and lease_evidence["status"] == "expired":
        ...
        interrupted = (closure["stop_requested"] or closure["forced_cleanup"]
                       or lease_evidence["status"] == "expired")
        ...
        # Interpretation and hashing run outside the writer but consume the
        # same lease. No effect intent exists yet, so known expiry may still
        # choose the narrow interrupted publisher and discard unregistered data.
        lease_evidence = _lease_observation(owner)
        if lease_evidence["status"] == "expired":
            outcome, prepared, domain_publication = "interrupted", None, None
        with execution_transaction(lake, owner.folder) as tx:
            ...
            tx.watch_cpu_runtime_lease(handle.attempt_ref, owner.binding["runtime_lease"],
                                       status=lease_evidence["status"])
            reason = ("cpu_runtime_lease_expired" if lease_evidence["status"] == "expired"
                      else "cpu_action_" + outcome)
            ...
            # This is the last admission gate after result registration, not
            # a promise of atomicity across the clock, SQL and filesystem.
