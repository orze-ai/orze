...
def _verify_record(conn, record):
    ...
    if record["outcome"]["status"] == "inserted":
        _lifecycle(conn, record["task_id"])


def _lifecycle(conn, task_id):
    # Only this coordinator's newly inserted tasks promise a creation event.
    # Ordinary legacy exact-ID replays never acquire invented FSM history.
    from orze.idea_lake import STATE_TO_STATUS
    fields = ("idea_id", "current_state", "updated_by_host", "updated_by_pid", "sop_type",
              "updated_at", "first_queued_at", "queued_at", "claimed_at", "started_at",
              "terminal_at", "completed_at")
    sizes = "+".join("coalesce(length(CAST(" + key + " AS BLOB)),0)" for key in fields)
    rows = conn.execute("SELECT " + ",".join(fields) + " FROM main.idea_state "
        "WHERE idea_id=? COLLATE BINARY AND (" + sizes + ")<=16384 LIMIT 2", (task_id,)).fetchall()
    mirror = conn.execute("SELECT CASE WHEN typeof(status)='text' AND "
                          "length(CAST(status AS BLOB))<=16 THEN status END "
                          "FROM main.ideas WHERE idea_id=? COLLATE BINARY LIMIT 2",
                          (task_id,)).fetchall()
    if (len(rows) != 1 or len(mirror) != 1 or type(rows[0][1]) is not str
            or rows[0][1] not in STATE_TO_STATUS or rows[0][4] != "action"
            or mirror[0][0] != STATE_TO_STATUS[rows[0][1]]):
        _fail("lifecycle_changed")
    latest = conn.execute("SELECT id,CASE WHEN typeof(to_state)='text' AND "
        "length(CAST(to_state AS BLOB))<=16 THEN to_state END,"
        "CASE WHEN typeof(sop_type)='text' AND length(CAST(sop_type AS BLOB))<=16 THEN sop_type END "
        "FROM main.idea_transitions "
        "WHERE idea_id=? COLLATE BINARY ORDER BY id DESC LIMIT 1", (task_id,)).fetchone()
    if (latest is None or type(latest[0]) is not int or latest[0] <= 0
            or latest[1] != rows[0][1] or latest[2] != "action"):
        _fail("lifecycle_transition_changed")
    return store.canonical({"state": dict(zip(fields, rows[0])), "status": mirror[0][0],
                            "latest": list(latest)})
    ...
    try:
        ...
        if previous is not None:
            ...
            if outcome["status"] == "inserted":
                staged_lifecycle = _lifecycle(conn, decision["task_id"])
        ...
        check_context()
        if (staged_lifecycle is not None
                and _lifecycle(conn, decision["task_id"]) != staged_lifecycle):
            _fail("staged_lifecycle_changed")
        conn.commit()
        if conn.in_transaction:
    ...
    def read(self, *, cursor=None, request_ids=None):
        ...
        try:
            with _readonly(self._route[1]) as conn:
                ...
                for row in view["requests"]:
                    record = store.get_request(conn, self._route[0], row["request_id"])
                    if record["database"] != self._route[1]:
                        _fail("historical_database_changed")
            self.verify()
