...
from orze.engine.controller_handoff import _Route, _validate_history, _schema
...
def verify_controller_record(cfg, proof):
    ...
    with route.connection() as conn:
        ...
        _validate_history(route, head[0], head[1], connection=conn)
...
def read_closed(service_config):
    ...
    if (owner.get('schema') != 1 or owner.get('service_sha256') != _sha(raw)
    ...
    proof = verify_controller_record(cfg, documents['closed.json'])
    route = _Route(cfg)
    with route.connection() as conn:
...
def inspect_closed(service_config):
    ...
    if type(proof) is not dict or proof.get('kind') != 'stopped':
        raise ControllerHOLD('service_closed_state_unconfirmed')
    return proof
