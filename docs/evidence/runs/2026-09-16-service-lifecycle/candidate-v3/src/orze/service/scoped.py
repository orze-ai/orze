...
def uninstall_units(svc, directory):
    ...
    def check_files():
        if any(_file_witness(directory / name, 65536)[0] != witness for name, witness in witnesses.items()):
            raise RuntimeError("service_unit_changed")
    _require_contract(svc)
    from orze.service.host import request, state_directory
    closed = state_directory(svc["service_config_file"]) / 'closed.json'
    snapshot = None
    if closed.exists() or closed.is_symlink():
        from orze.service.closed_state import inspect_closed
        snapshot = inspect_closed(svc['service_config_file'])
    ...
    for name in unit_names(svc):
        ...
        if snapshot is None:
            ...
            if inspect_closed(svc['service_config_file']) != snapshot:
                raise RuntimeError('service_closed_state_changed')
            _require_inactive(svc)
            # An on-disk closure record never grants process control.
            # Disable startup links only; never stop or signal a dead PID.
            _checked('disable', name)
    check_files()
...
def _require_inactive(svc):
    """Observe the exact inactive units before metadata-only removal."""
    from orze.service.runtime_contract import _systemd_properties, _unit_errors
    main, timer, watchdog = unit_names(svc)
    units = render_units(svc)
    def lines(raw):
        if type(raw) is not str:
            raise RuntimeError('service_closed_unit_text_unconfirmed')
        return [line.strip() for line in raw.splitlines() if line.strip() and not line.lstrip().startswith('#')]
    for unit in (main, timer, watchdog):
        props = _systemd_properties(unit, kind='timer') if unit == timer else _systemd_properties(unit)
        if (props.get('Id') != unit or props.get('ActiveState') != 'inactive'
                or lines(props.get('_UnitText')) != lines(units[unit])):
            raise RuntimeError('service_closed_unit_state_unconfirmed')
        if unit == timer:
            if props.get('Unit') != watchdog:
                raise RuntimeError('service_closed_timer_target_changed')
        elif (props.get('MainPID') != '0' or props.get('ControlGroup') != ''
              or _unit_errors(svc, props, watchdog=unit == watchdog)):
            raise RuntimeError('service_closed_unit_process_unconfirmed')
