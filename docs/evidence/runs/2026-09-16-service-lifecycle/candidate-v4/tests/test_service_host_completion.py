    ...
    try:
        with sqlite3.connect(root / 'lake.db') as conn:
            if mutation == 'ack_missing':
                ...
                # Deliberately corrupt closed storage; the public reserve API
                # correctly refuses work after the budget has stopped.
                scope = json.loads(conn.execute('SELECT binding_json FROM cpu_action_scopes').fetchone()[0])
                permit = {'schema': 1, 'budget_scope': scope, 'reservation_id': 'f' * 48,
                          'task_id': 'unregistered-action', 'slot': 0,
                          'wall_limit_seconds': 1, 'reserved_nanoseconds': '1000000000'}
                conn.execute("INSERT INTO cpu_action_reservations VALUES (?,?,?,?,?,NULL,'RESERVED',NULL)",
                             (permit['reservation_id'], scope['results_dir'], permit['task_id'], 0,
