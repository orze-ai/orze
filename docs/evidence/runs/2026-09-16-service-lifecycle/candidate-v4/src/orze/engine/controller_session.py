...
class ControllerSession:
    ...
    def bind_cpu_budget(self, scope):
        from orze.core.cpu_execution import cpu_controller_profile
        from orze.core.cpu_action_budget import _check_route, _json
        with self.ctx.guard():
            if (not cpu_controller_profile(self.orze.cfg) or self._cpu_scope_json is not None
                    or scope is not self.orze._cpu_scope):
                raise ControllerHOLD("controller_cpu_resource_invalid")
            self._cpu_scope_json = _json(_check_route(self._lake, scope))

    def _cpu_proof(self, conn):
        from orze.core.cpu_execution import cpu_execution, cpu_handoff_profile
        from orze.core.cpu_action_budget import _json
        from orze.engine.controller_cpu import budget_drain_proof, budget_history_proof
        if (self._cpu_scope_json is None or self._cpu_scope_json != _json(self.orze._cpu_scope)
                or self.orze._cpu_handles or self._gpu is not None or self.orze._gpu_leases is not None):
            raise ControllerHOLD("controller_cpu_resource_unconfirmed")
        prove = budget_history_proof if cpu_handoff_profile(self.orze.cfg) else budget_drain_proof
        return prove(conn, self.orze.results_dir, cpu_execution(self.orze.cfg), self.ctx.controller_id)
    ...
    def _close_resources(self):
        ...
        cpu = None
        if cpu_controller_profile(self.orze.cfg):
            with self.ctx._connection() as conn:
                conn.execute("BEGIN")
                cpu = self._cpu_proof(conn)
        self._lake.close()
        try:
        ...
        resources = {"lake": "closed", "gpu_scope": sorted(self.orze.gpu_ids),
    ...
    def finish(self):
        ...
        try:
            ...
            with self.ctx.guard():
                ...
                encoded = _encode(ack)
                with self._connection(write=True) as (conn, row):
                    if ("cpu_budget" in resources and
                            self._cpu_proof(conn) != resources["cpu_budget"]):
                        raise ControllerHOLD("controller_cpu_budget_changed_before_ack")
...
class _Observer:
    def __init__(self, cfg, *, _children=None):
        from orze.core.controller_profile import controller_profile, profile_fingerprint
        ...
        self._cpu_declaration = cpu_execution(cfg) if cpu_controller_profile(cfg) else None
        ...
        try:
            if _children is None:
                ...
                from orze.engine.controller_handoff import _ChildSet
                if type(_children) is not _ChildSet:
                    raise ControllerHOLD("controller_stop_child_owner_invalid")
                self.pidfd = _children.capture(process)
            self.check()
    ...
    def verify_drain(self, ack, *, conn=None):
        ...
        from orze.core.cpu_execution import cpu_controller_profile, cpu_handoff_profile
        profile_cfg = {"controller_control": self.binding["profile"]}
        cpu = cpu_controller_profile(profile_cfg)
        resource_fields = {"lake", "gpu_scope", "gpu_leases", "pid_file", "leadership", "request_pump"}
        if cpu:
            resource_fields.add("cpu_budget")
        ...
        with self.connection() if conn is None else nullcontext(conn) as conn:
            ...
            if cpu:
                from orze.engine.controller_cpu import budget_drain_proof, budget_history_proof
                if cpu_handoff_profile(profile_cfg):
                    actual = budget_history_proof(conn, self.scope, self._cpu_declaration,
                        self.controller_id, closed_inventory=not self._history_only)
                else:
                    actual = budget_drain_proof(conn, self.scope, self._cpu_declaration, self.controller_id)
                if canonical(actual) != canonical(resources["cpu_budget"]):
                    raise ControllerHOLD("controller_cpu_budget_proof_changed")
            member_schema(conn)
...
def _complete_observed(observer, registration, row, request_json):
    """Verify an existing ACK and captured kernel exit; never request work."""
    ...
    if not observer.exited():
        return None
    # Re-read after observing kernel exit; no stale request, ACK or resources.
...
def _closed_observed(observer):
    """Read closure of a previously captured process; no historical PID lookup."""
    if (type(observer) is not _Observer or observer._history_only or observer.pidfd is None
            or not observer.exited()):
        raise ControllerHOLD("controller_closed_process_unconfirmed")
    registration, row = observer.check()
    if row[2] is None or row[3] is None:
        raise ControllerHOLD("controller_exited_without_ack")
    return _complete_observed(observer, registration, row, row[2])
...
def _stop_observed(observer, timeout):
    ...
    while True:
        ...
        if row[3] is not None:
            completed = _complete_observed(observer, registration, row, request_json)
