...
def _is_systemd_active(unit="orze.service"):
...
def _is_systemd_timer_active(unit="orze-watchdog.timer"):
...
def show_status(service_config_file=None):
    ...
    if service_config_file is None:
        ...
        svc_cfg = selected(service_config_file)
    if not svc_cfg:
    ...
    if method == "crontab":
        ...
        main_unit, timer_unit, _ = unit_names(svc_cfg)
        svc_active = _is_systemd_active() if "service_owner" not in svc_cfg else _is_systemd_active(main_unit)
        timer_active = _is_systemd_timer_active() if "service_owner" not in svc_cfg else _is_systemd_timer_active(timer_unit)
        print(f"    {_ok(main_unit + ' active') if svc_active else _no(main_unit + ' not active')}")
        print(f"    {_ok(timer_unit + ' active') if timer_active else _no(timer_unit + ' not active')}")
    ...
    if "service_owner" in svc_cfg:
        ...
        try:
            observed = request(svc_cfg["service_config_file"], "status", timeout=5)
            label = 'Completed controller ' if observed.get('state') == 'stopped' else 'Registered controller '
            print(f"    {_ok(label + observed['controller']['controller_id'])}")
        except Exception:
            try:
                from orze.service.closed_state import inspect_closed
                closed = inspect_closed(svc_cfg['service_config_file'])
                print(f"    {_ok('Verified closure record for controller ' + closed['closure']['controller_id'])}")
            except Exception:
                print(f"    {_warn('Registered service host status unconfirmed')}")
        pid = None
    else:
    ...
    elif "service_owner" not in svc_cfg:
...
def show_logs(n=50, service_config_file=None):
