...
from orze.core.controller_profile import (
    ...
    validate_successor_cli, handoff_profile,
)
...
Examples:
  ...
  python -m orze.cli --report-only               # regenerate report
    ...
    svc_install.add_argument("--service-config", help="New explicit configuration for an isolated registered service")

    svc_recover = svc_sub.add_parser('recover', help='Prepare a new host from a verified closed CPU service')
    svc_recover.add_argument('--source-service-config', required=True)
    svc_recover.add_argument('--service-config', required=True, help='New configuration path; old records are retained')
    ...
    svc_upgrade.add_argument('--install', action='store_true', help='Also install/start the new systemd service')
    ...
    for selected_service in (svc_uninstall, svc_status, svc_audit, svc_logs):
        selected_service.add_argument("--service-config", help="Select one isolated service configuration")
    ...
    if command == "service":
        ...
        if action == "install":
            ...
            options = {"method": args.method, "stall_threshold": args.stall_threshold}
            if args.service_config is not None:
                options["service_config_file"] = args.service_config
            install(args.config_file, **options)
        elif action == "uninstall":
            from orze.service.install import uninstall
            uninstall() if args.service_config is None else uninstall(service_config_file=args.service_config)
        elif action == 'recover':
            from orze.service.recovery import main as recover_main
            ...
            return audit_main([] if args.service_config is None else ["--service-config", args.service_config])
        elif action == "capture-runtime":
            from orze.service.runtime_contract import main as audit_main
            return audit_main(["--capture-controller"])
        elif action == "logs":
            from orze.service.status import show_logs
            if args.service_config is None:
                show_logs(n=args.n)
            else:
                show_logs(n=args.n, service_config_file=args.service_config)
        else:
    ...
    if "ORZE_SERVICE_HOST_INITIAL_FD" in os.environ:
        from orze.service.host import prepare_initial_entry
        try:
            prepare_initial_entry(cfg)
        except Exception:
            print("HOLD: service host initial admission is unconfirmed")
            return 75
    ...
    if cpu:
        ...
        try:
            ...
            orze.run()
        except (CPUExecutionError, CpuBudgetHOLD, CPUActionHOLD, ControllerHOLD) as exc:
