"""Replicate the same Haiku A/B design after the observed duplicate-admission harness fix."""
import hashlib
import json
import os
...
pilot.credentials()
output=pilot.ROOT/'real-memory-pilot-20260916-admission'
output.mkdir(mode=0o700)
budget={'authorized_total_usd':5000,'prior_reserved_upper_bound_usd':213,'new_upper_bound_usd':32,
        'total_reserved_upper_bound_usd':245,
        'arithmetic':'8 projects x 4 processes x 100000-token envelope x $5/M maximum rate = $16; reserve $32',
        'scope':'same 4 paired Haiku tasks after duplicate-admission harness fix; no post-result resampling',
...
previous=pilot.ROOT/'real-memory-pilot-20260916-fixed/haiku45-specification.json'
spec=json.loads(previous.read_bytes())
runtime=campaign.describe()
spec['shared']['environment']=runtime['environment']
plan=spec['protocol']
plan['comparison_id']='real-memory-haiku45-admissionfix'
