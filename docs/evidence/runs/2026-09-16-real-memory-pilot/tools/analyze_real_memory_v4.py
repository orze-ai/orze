...
FIXED = before.ROOT / 'real-memory-pilot-20260916-admission'
PARSER = before.ROOT / 'real-memory-pilot-20260916-fixed'
...
def main():
    ...
    for line in (FIXED / 'haiku45/runs.jsonl').read_text().splitlines():
        ...
        outer_started = before.read(directory/'run.json')['started_monotonic']
        optimum = 27 if row['task_id'] == 'memory-target' else 10
        row['first_optimal_consumed_seconds'] = next((receipt['observed_monotonic'] - outer_started
            for receipt in capture['campaign']['consumption']
            if receipt['evaluation']['verdict']['status'] == 'valid'
            and receipt['evaluation']['verdict']['scheduled_value'] == optimum), None)
    ...
    for task in ('memory-target', 'memory-negative-control'):
        for repetition in (0, 1):
            ...
            item = {'task_id': task, 'repetition': repetition, 'equal_confirmed_quality': equal_quality,
                    'A': {k: a[k] for k in ('run_id', 'quality', 'wall_seconds', 'first_optimal_consumed_seconds', 'metrics', 'usage')},
                    'B': {k: b[k] for k in ('run_id', 'quality', 'wall_seconds', 'first_optimal_consumed_seconds', 'metrics', 'usage')}}
            ...
            pairs.append(item)
    journals = sorted(p for root in (before.FIRST, before.CURRENT, PARSER, FIXED) for p in root.rglob('usage.jsonl'))
    quantities = [before.usage(p) for p in journals]
    result = {'scope': baseline['scope'], 'money_scope': baseline['money_scope'],
        'total_budget_usd': 5000, 'conservative_reserved_upper_bound_usd': 245,
        ...
        'parser_fix_haiku45': before.read(PARSER/'independent-summary-v3.json')['after_fix_haiku45'],
    ...
    with (FIXED/'independent-summary-v4.json').open('x') as stream:
