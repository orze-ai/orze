...
CORE = ROOT / 'orze'
PRO = ROOT / 'pro'
OUTPUT = ROOT / 'real-memory-pilot-20260916'
...
def prepare():
    ...
    write(OUTPUT / 'authorization-and-budget.json', {
        ...
        'total_cap_usd': 5000, 'reserved_upper_bounds_usd': RESERVES,
        ...
        'initial_source_heads': {'core': '53608e2b26974c070c2b11c583b8fb860e3d8802', 'pro': '8b08097218759c16862751c733a322a5dca84271'},
    ...
    for label, model in MODELS.items():
        tools = {'workload': 'scheduling-v1', 'rounds': 4, 'num_ideas': 2,
                 'evaluation_protocol': 'schedule-feasibility-v1'}
        treatments = {}
        for arm in ('A', 'B'):
        ...
        for name, instance in cases.items():
            ...
            tasks.append({'id': task_id, 'domain': 'scheduling', 'role': name, 'seeds': [17, 41],
                          ...
                          'limits': {'provider_calls': 24, 'provider_tokens': 400000, 'provider_cost_usd': RESERVES[label]/8,
                                     'reserved_seconds': 40, 'gpu_seconds': 0, 'cli_wall_seconds': 600}})
...
def execute(label):
    ...
    assert sum(budget['reserved_upper_bounds_usd'].values()) <= budget['total_cap_usd']
