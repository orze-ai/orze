...
def cpu_execution(cfg):
    ...
    result = dict(value)
    if ((cfg.get("controller_control") is not None and not cpu_stop_profile(cfg))
...
def validate_cpu_cli(cfg, args):
    ...
    for name in ("gpus", "role_only", "train_script", "base_config", "timeout",
        ...
        if (name == "timeout" and cpu_stop_profile(cfg) and
                (getattr(args, "command", None) == "stop" or getattr(args, "stop", False))):
            continue
