...
class ControllerSession:
    ...
    def bind_cpu_budget(self, scope):
        from orze.core.cpu_execution import cpu_stop_profile
        from orze.core.cpu_action_budget import _check_route, _json
        with self.ctx.guard():
            if (not cpu_stop_profile(self.orze.cfg) or self._cpu_scope_json is not None
    ...
    def _cpu_proof(self, conn):
        ...
        from orze.engine.controller_cpu import budget_drain_proof
        ...
        return budget_drain_proof(conn, self.orze.results_dir,
                                  cpu_execution(self.orze.cfg), self.ctx.controller_id)
    ...
    def _close_resources(self):
        ...
        cpu = None
        if cpu_stop_profile(self.orze.cfg):
            with self.ctx._connection() as conn:
                conn.execute("BEGIN")
...
class _Observer:
    def __init__(self, cfg):
        ...
        from orze.core.cpu_execution import cpu_execution, cpu_stop_profile
        self._cpu_declaration = cpu_execution(cfg) if cpu_stop_profile(cfg) else None
    ...
    def verify_drain(self, ack, *, conn=None):
        ...
        cpu = self.binding["profile"] == {"version": 1, "profile": "local_cpu_stop_v1"}
