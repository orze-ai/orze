...
from test_cpu_action_budget import context
...
def test_cpu_handoff_replays_once_and_keeps_history_after_new_work(project, continued):
    from orze.core.config import load_project_config
    ...
    try:
        ...
        original_ack = rows(root, 'SELECT ack_json FROM controller_sessions WHERE controller_id=' +
                            repr(source_id))[0][0]
        # A new queued task uses normal ingress. Handoff does not clear the
        # old task's retained claim or grant an implicit retry of failed work.
        action = {'version': 1, 'adapter': 'command', 'purpose': 'successor CPU work',
                  'inputs': {}, 'command': [sys.executable, '-c', 'pass'],
                  'timeout_seconds': 60, 'outputs': {}}
        with (root / 'ideas.md').open('a') as stream:
            stream.write('\n## idea-0002: successor action\n\n```yaml\n' +
        ...
        assert rows(root, 'SELECT task_id,state FROM execution_attempts ORDER BY task_id') == [
            ('idea-0001', 'TERMINAL'), ('idea-0002', 'TERMINAL')]
        ...
        (root / 'product-summary.json').write_text(json.dumps({
            'controller_generations': 3, 'native_attempts': 2, 'task_generations': [1, 1],
            'successors': 2, 'replay_extra_successors': 0, 'charged_nanoseconds': 120_000_000_000,
            'old_tree_exited_before_successor': True, 'continued_authorization': continued}))
    finally:
        for fd in fds:
            os.close(fd)


@pytest.mark.parametrize('fault', ['orphan_at_commit', 'lost_started_reply', 'policy_stop_at_commit'])
def test_cpu_handoff_unknown_commit_or_reply_never_spawns_again(project, fault):
    ...
    try:
        ...
        if fault in {'orphan_at_commit', 'policy_stop_at_commit'}:
            ...
            if fault == 'orphan_at_commit':
                ...
                assert rows(root, 'SELECT state FROM cpu_action_reservations') == []
                assert json.loads(rows(root, 'SELECT stop_json FROM cpu_action_scopes')[0][0])['reason'] == 'fixture_policy_stop'
...
def test_historical_cpu_terminal_does_not_authorize_live_settlement(context):
    """Real no-execution receipts; this is metadata verification, not handoff."""
    import hashlib
    ...
    c = context
    first = budget.reserve(c.lake, c.scope, 'task', 2)
    folder, ref = _intent(c, first)
    terminal = _not_started(c, folder, ref)
    assert budget.settle(c.lake, first, ref, terminal) == 'settled'
    second = budget.reserve(c.lake, c.scope, 'task', 2)
    with execution_transaction(c.lake, folder) as tx:
        newer = attempts.create_attempt(tx.conn, 'task', 'action', 'newer-attempt',
                                         {'reservation_id': second['reservation_id']})
        tx.watch_attempt(newer)
    budget.bind(c.lake, second, newer)
    newer_terminal = _not_started(c, folder, newer)
    assert budget.settle(c.lake, second, newer, newer_terminal) == 'settled'
    assert newer.generation == ref.generation + 1
    old = attempts._row(c.lake.conn.execute(attempts._SELECT + ' WHERE attempt_id=?',
                                            (ref.attempt_id,)).fetchone())
    assert budget._terminal_record(first, ref, old, terminal) == hashlib.sha256(
        attempts._json(terminal).encode()).hexdigest()
    with pytest.raises(attempts.StaleAttempt):
        budget._terminal(c.lake.conn, first, ref, terminal)
    assert budget.snapshot(c.lake, c.scope)['reserved_wall_seconds'] == 4


@pytest.mark.parametrize('fault', ['writer_settlement', 'effect_receipt'])
def test_cpu_handoff_history_rechecks_budget_and_receipt_in_actual_view(project, fault):
    from orze.core.config import load_project_config
    from orze.engine.controller_control import ControllerHOLD
    from orze.engine.controller_handoff import _Route, _validate_history
    root, path, cfg, env, launch, wait_for, control, births, expected_old = project
    old = launch('pass', once=True)
    assert old.wait(timeout=15) == 0, (root / 'controller-0.log').read_text()
    route = _Route(load_project_config(str(path)))
    source_id = rows(root, 'SELECT controller_id FROM controller_instances')[0][0]
    digest = _validate_history(route, source_id, 0)
    if fault == 'writer_settlement':
        with route.connection(write=True) as conn:
            ...
            with pytest.raises(ControllerHOLD, match='cpu'):
                _validate_history(route, source_id, 0, expected=digest, connection=conn)
            assert conn.in_transaction
            assert conn.execute('SELECT terminal_sha256 FROM cpu_action_reservations').fetchone()[0] == '0' * 64
            conn.rollback()
        assert _validate_history(route, source_id, 0, expected=digest) == digest
    else:
        receipt = next((root / 'results/idea-0001/_execution_effects').glob('*/prepared.json'))
        raw = receipt.read_bytes()
        (root / 'original-effect.json').write_bytes(raw)
        receipt.write_bytes(raw + b' ')
        with pytest.raises(ControllerHOLD):
            _validate_history(route, source_id, 0, expected=digest)
        assert receipt.read_bytes() == raw + b' '
    assert not births
    (root / 'history-control.json').write_text(json.dumps({'fault': fault,
        'refused': True, 'native_attempts': 1, 'new_launch_authority': False}))
