...
def initialize(engine, gpu_ids, cfg, once):
    ...
    engine._controller_profile_enabled = controller_profile(cfg) is not None
...
def _require_invocation(engine):
    ...
    from orze.core.control_outcome import require_controller_start_allowed
    session = getattr(engine, "_controller_session", None)
...
def start(engine):
    ...
    session = getattr(engine, "_controller_session", None)
    if session is not None:
        session.bind_cpu_budget(engine._cpu_scope)
    ...
    if session is not None and session._admission is not None:
        # This is the original start call, before any policy or worker runs.
        # Resource readiness waits for the issuer's one-shot COMMIT here.
...
def _drain_actions(engine):
    """Retain the Lake until the session has verified settlement and closed it."""
    _release_evidence_scan(engine)
...
def close(engine):
    ...
    if failures:
        raise CPUExecutionError("execution: CPU cleanup remains unconfirmed: " + ",".join(failures))
    _release_invocation(engine)


def _release_invocation(engine):
...
def run_registered(engine):
    """Register the existing CPU loop and attest its complete owned shutdown."""
    from orze.engine.controller_control import ControllerQuiescing
    from orze.engine.controller_session import ControllerSession
    session = None
    try:
        session = ControllerSession(engine)
        engine._controller_session = session
        session.start()
        try:
            session.ctx.check_admission()
            engine._run_leased()
            ...
            pass
        session.request_local_stop("normal_exit")
        _drain_actions(engine)
