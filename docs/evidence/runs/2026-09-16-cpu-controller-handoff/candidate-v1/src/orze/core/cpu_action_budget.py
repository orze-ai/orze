...
def _terminal(conn, permit, ref, evidence):
    ...
    return _terminal_record(permit, ref, row, evidence)


def _terminal_record(permit, ref, row, evidence):
    """Verify one exact closed record; this alone grants no settlement authority.

    Live settlement still enters through _terminal's require_current check.
    Handoff history can verify a sealed older generation without adopting it.
    """
    from orze.engine.attempt_effect_receipts import _scan, _read as effect_read, _decode as effect_decode, _ref_fields
    scope = permit["budget_scope"]
    if (row is None or {key: row[key] for key in ("task_id", "phase", "attempt_id", "generation")}
            != asdict(ref) or row["state"] not in {"TERMINAL", "NOT_STARTED"}
            or row["hold_reason"] is not None
            or row["binding"].get("reservation_id") != permit["reservation_id"]):
        _fail("terminal_reference_changed")
