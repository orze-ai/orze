...
CPU_STOP_PROFILE = {"version": 1, "profile": "local_cpu_stop_v1"}
CPU_HANDOFF_PROFILE = {"version": 2, "profile": "local_cpu_handoff_v1"}


def cpu_stop_profile(cfg):
    value = cfg.get("controller_control")
    return (type(value) is dict and value == CPU_STOP_PROFILE
            and type(value.get("version")) is int)


def cpu_handoff_profile(cfg):
    value = cfg.get("controller_control")
    return (type(value) is dict and value == CPU_HANDOFF_PROFILE
            and type(value.get("version")) is int)


def cpu_controller_profile(cfg):
    return cpu_stop_profile(cfg) or cpu_handoff_profile(cfg)
...
def cpu_execution(cfg):
    ...
    result = dict(value)
    if ((cfg.get("controller_control") is not None and not cpu_controller_profile(cfg))
            or cfg.get("_managed_idea_id")):
...
def execution_fingerprint(cfg, *, declaration=None):
    ...
    raw = json.dumps({"execution": value, "action_policy": action_policy(cfg),
                      "controller_control": cfg.get("controller_control"),
...
def validate_cpu_cli(cfg, args):
    ...
    handoff = cpu_handoff_profile(cfg)
    ...
    if (command in ("run-idea", "start", "resume", "enable", "upgrade")
            or (command == "restart" and not handoff)):
    ...
    for name in ("gpus", "role_only", "train_script", "base_config", "timeout",
        ...
        if (name == "timeout" and cpu_controller_profile(cfg) and
                (command == "stop" or getattr(args, "stop", False)
                 or (handoff and (command == "restart" or getattr(args, "restart", False))))):
            continue
