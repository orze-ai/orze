...
def main():
    ...
    final = read(raw / 'product-v3-fixture/result.json')
    assert baseline['filesystem'] == final['filesystem'] == 'ceph'
    assert all(r['stats']['errors'] == {'checkpoints':1,'results':1,'archive':2}[r['mode']] and r['stats']['reasons'] == ['storage_atomic_rename_unsupported']
               for r in baseline['records'])
    expected = {'checkpoints': ('deleted',1,16), 'results': ('deleted_files',1,12), 'archive': ('archived_files',2,32)}
    for run in final['records']:
        project = raw / 'product-v3-fixture' / run['mode']
    ...
    cost = read(raw / 'cost-v2-fixture/result.json')
    ...
    admission = Path('/tmp/gg-t5/test_fresh_claim_is_denied_whi0')
