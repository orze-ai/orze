...
actual_listdir = history._accessor.listdir
accessor = type(history._accessor)
...
def observed(path):
    ...
    return result
accessor.listdir = staticmethod(observed)
class ObservedScan:
    def __init__(self, path):
        self.path = Path(path)
        self.scan = actual_scandir(path)
    def __enter__(self):
        return self
...
try:
    ...
    accessor.listdir = staticmethod(actual_listdir)
    os.scandir = actual_scandir
assert refused
report = {'archive_errors':stats['errors'], 'leaked_descriptors':len(leaked),
          ...
          'eager_directory_entries':materialized, 'scandir_entries':len(visited), 'reader_refused':refused,
...
raise SystemExit(0 if not leaked and max(materialized, default=0) <= 1025 and len(visited) <= 1025 else 1)
