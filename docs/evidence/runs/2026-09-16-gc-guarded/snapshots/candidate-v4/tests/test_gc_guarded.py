...
'''
    ...
    assert observed == {'claimed': not blocked, 'reason': 'gc_retirement_unconfirmed' if blocked else None,
                        'claim_exists': not blocked}
    return observed
...
def test_large_inventory_does_not_use_small_control_record_limit(tmp_path):
    scope, task, checkpoint = guarded(tmp_path)
    for i in range(600):
        (checkpoint / ('entry-' + str(i))).write_bytes(b'owned')
    result = run(scope)
