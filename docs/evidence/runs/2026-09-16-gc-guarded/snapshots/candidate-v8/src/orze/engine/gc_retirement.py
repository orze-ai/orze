...
def _entries(path, limit):
    # Path.iterdir materializes the full directory on supported Python 3.10.
    # Bound consumption as well as retention, and always close the iterator.
    with os.scandir(path) as entries:
        return [Path(entry.path) for entry in itertools.islice(entries, limit)]
...
def _records(folder):
    ...
    records = _entries(root, _LIMIT + 1)
    ...
    for record in records:
        ...
        names = {p.name for p in _entries(record, 3)}
...
class Retirement:
    def __init__(self, folder, record, raw, journal):
        self.folder, self.record, self.raw = folder, record, raw
        self.journal = Path(journal)
        paths = {folder, record.parent, record, self.journal, *folder.parents, *self.journal.parents}
        self.identities = {p: _identity(p) for p in paths}
        from orze.engine.attempt_effect_receipts import _read
        self.journal_intent = _read(self.journal / 'intent.json')

    def check(self):
        from orze.engine.attempt_effect_receipts import _read
        if (any(_identity(path) != expected for path, expected in self.identities.items())
                or _read(self.record / 'intent.json') != self.raw
                or _read(self.journal / 'intent.json') != self.journal_intent
                or {p.name for p in _entries(self.record, 3)} != {'intent.json'}):
...
def prepare(lease, journal, plan_sha256):
    ...
    result = Retirement(folder, record, raw, journal)
    result.check()
    return result
