...
@pytest.mark.parametrize('after_write', [1, 2])
def test_archive_manifest_loop_stops_before_next_write(tmp_path, monkeypatch, change, after_write):
    scope, task, checkpoint = guarded(tmp_path)
    (task / 'scratch.pt').unlink()
    overlay = task / 'overlays'
    overlay.mkdir()
