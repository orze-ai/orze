...
Atomic mode reclaims after same-root quarantine detachment. Explicit guarded
mode retires task effects durably before bulk copying/removal outside the guard.
"""
...
def gc_scope(results_dir, cfg, *, lake=None, lake_db_path=None,
    ...
    cfg = deepcopy(cfg)
    from orze.engine.storage_preflight import gc_storage_mode
    gc_storage_mode(cfg)
...
def _guarded_running(scope, folder):
    ...
    for name in ('.orze_disabled', '.orze_stop_all', '.orze_shutdown'):
        latch = scope.results_dir / name
        if latch.exists() or latch.is_symlink():
            raise GCRefused('gc_guarded_stop_latch')
...
def collect(scope, root, keep_ids, *, mode, dry_run=False):
    ...
    from orze.engine.storage_preflight import gc_storage_mode, require_gc_storage
    guarded = gc_storage_mode(scope.cfg) == 'guarded'
    from orze.engine import gc_retirement
    ...
    try:
        pending_root = root / "_orze_gc_quarantine"
        plain_directory(pending_root, missing=True)
        if pending_root.exists():
            pending_tasks = _entries(pending_root)
            if len(pending_tasks) > 1024:
                raise GCRefused("gc_quarantine_limit")
            for pending_task, _ in pending_tasks:
                require_no_pending(root, pending_task.name)
                gc_retirement.require_quiet(scope.results_dir / pending_task.name)
        candidates = _candidates(scope, mode)
    except Exception as exc:
        stats.update(errors=1, reasons=[str(exc) if isinstance(exc, GCRefused) else "gc_scan_unavailable"])
        return stats
    for task_id, path, captured in candidates:
        ...
        try:
            ...
            plan = None
            if guarded:
                from orze.engine.gc_guarded import plan_bytes
                plan = plan_bytes(tree, destination)
            require_no_pending(root, task_id)
            gc_retirement.require_quiet(folder)
            if dry_run:
            ...
            require_gc_storage(root, scope.cfg)
            require_gc_storage(path.parent, scope.cfg)
            if destination is not None:
                ...
                require_gc_storage(archive_parent, scope.cfg)
            with attempt_effect_lock(folder) as lease:
                with _authority(folder, scope.cfg, scope.lake) as conn:
                    ...
                    try:
                        if guarded:
                            _guarded_running(scope, folder)
                            retirement = gc_retirement.prepare(lease, directory, hashlib.sha256(plan).hexdigest())
                        else:
            ...
            if retirement is not None:
                from orze.engine.gc_guarded import execute
                def check():
                    retirement.check()
                    _require_scope(scope, root, mode)
                    _guarded_running(scope, folder)
                try:
                    execute(tree, directory, destination, check, plan)
                    retirement.complete()
                except BaseException as exc:
                    raise AttemptEffectInDoubt('gc_guarded_unconfirmed') from exc
            else:
