...
out=ROOT/'observation-restored-check-20260917'
out.mkdir(mode=0o700)
(out/'tmp').mkdir()
names=['pro/src/orze_pro/agents/report_paging.py','pro/src/orze_pro/agents/joint_snapshot.py','pro/tests/test_native_cpu_research_feedback.py','orze/src/orze/engine/cpu_phase.py']
def hashes():return {n:hashlib.sha256((ROOT/n).read_bytes()).hexdigest() for n in names}
before=hashes()
args=['-q','tests/test_native_cpu_research_feedback.py','--tb=short','-p','no:cacheprovider','--basetemp='+str(out/'pytest'),'--junitxml='+str(out/'junit.xml')]
