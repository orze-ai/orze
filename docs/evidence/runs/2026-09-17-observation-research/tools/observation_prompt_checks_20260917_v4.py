...
out=ROOT/'observation-prompt-check-20260917-v4'
...
before=hashes()
args=['-q','tests/test_joint_research_snapshot.py','--tb=short','-p','no:cacheprovider','--basetemp='+str(out/'pytest'),'--junitxml='+str(out/'junit.xml')]
...
value={'exit_code':result.returncode,'seconds':time.monotonic()-started,'before':before,'after':hashes(),'args':args,'scope':'Offline provider fixtures and test-only license patch; joint snapshot fixtures; no native worker or external provider call in this final wording check.'}
