...
BATCH=ROOT/'observation-research-20260917-v2'
prior.BATCH=BATCH
read=prior.read
...
def main():
    ...
    for process in processes:
        ...
        assert frozen['request']['inputs']['data']==task['data']
        assert frozen['effective_prompt_source_sha256']==plan['source_sha256']
        assert frozen['predeclared_plan_sha256']==hashlib.sha256((ROOT/'observation_research_plan_20260917_v2.json').read_bytes()).hexdigest()
        ...
        seen=set();repeated=[];unique_improvements=[]
        for evaluation in row['evaluations']:
            canonical=json.dumps(sorted((x['job_id'],x['start']) for x in evaluation['schedule']))
            if not evaluation['seed']:
                if canonical in seen: repeated.append(evaluation['task_id'])
                elif evaluation['verdict'].get('status')=='valid' and evaluation['verdict'].get('scheduled_value',0)>row['seed_score']:unique_improvements.append(evaluation['task_id'])
            seen.add(canonical)
        row['repeated_evaluated_schedules']=repeated
        row['distinct_valid_improvements']=unique_improvements
    ...
    for arm in ('A','B'):
        ...
        arms.append(dict(arm=arm,projects=len(selected),improved=sum(r['improved'] for r in selected),target_reached=sum(r['target_reached'] for r in selected),
            distinct_valid_improvements=sum(len(r['distinct_valid_improvements']) for r in selected),repeated_evaluated_schedules=sum(len(r['repeated_evaluated_schedules']) for r in selected),
    ...
    previous=read(ROOT/'observation-research-20260917/summary.json')['cumulative_usage']
    cumulative={k:previous[k]+sum(r['usage'][k] for r in rows) for k in previous}
    result=dict(schema=1,scope=plan['scope'],arms=arms,pairs=pairs,runs=rows,cumulative_usage=cumulative,cumulative_reserve_usd=395,authorized_cap_usd=5000,limitations=plan['limitations'],
