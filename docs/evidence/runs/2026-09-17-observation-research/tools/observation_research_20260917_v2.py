...
def main():
    ...
    pilot.credentials()
    root = pilot.ROOT / 'observation-research-20260917-v2' / f'pair-{args.repetition}-{args.arm}'
    root.mkdir(mode=0o700)
    plan = json.loads((pilot.ROOT/'observation_research_plan_20260917_v2.json').read_bytes())
    budget = {'authorized_total_usd': 5000, 'reserved_for_slot_usd': 3,
              'whole_batch_reserved_usd': 24, 'cumulative_reserved_usd': 395,
    ...
    for path, digest in plan['core_source_sha256'].items():
        assert hashlib.sha256((pilot.CORE/path).read_bytes()).hexdigest() == digest
...
"""
    ...
    pilot.write(root/'plan.json', {'request': request, 'budget': budget,
        ...
        'predeclared_plan_sha256': hashlib.sha256((pilot.ROOT/'observation_research_plan_20260917_v2.json').read_bytes()).hexdigest(),
