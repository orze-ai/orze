...
def _guarded_role_namespace(lock_dir: Path) -> bool:
    marker = Path(lock_dir).with_name(Path(lock_dir).name + ".role-lock")
...
def _fs_lock(lock_dir: Path, stale_seconds: float = 600) -> bool:
    ...
    if idea_source_lock_protected(lock_dir) or _guarded_role_namespace(lock_dir):
...
def _fs_unlock(lock_dir: Path):
    ...
    if _guarded_role_namespace(lock_dir):
        return
    try:
