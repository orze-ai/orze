...
class RoleLaunch:
    ...
    def __init__(self, meta, lock_identity, lock_bytes, trigger_token, storage_lease=None):
    ...
    def _current(self):
        try:
            ...
            if self._storage_lease is not None:
                from orze.engine.role_storage import require_guarded_role
                require_guarded_role(self._storage_lease)
    ...
    def _require_release(self, outcome, exit_code):
        self._current()
        if self._held_reason:
    ...
    def release(self, *, outcome=None, exit_code=None):
        ...
        try:
            if self._storage_lease is not None:
                from orze.engine.role_storage import release_guarded_role
                release_guarded_role(self._storage_lease, self, outcome=outcome, exit_code=exit_code)
            else:
                self._require_release(outcome, exit_code)
...
def begin_role_launch(metadata, *, storage_lease=None):
    owner = None
    try:
        ...
        from orze.core.fs import _guarded_role_namespace
        if storage_lease is None and _guarded_role_namespace(lock):
            raise ValueError("role_guarded_storage_lease_required")
        if storage_lease is not None:
            from orze.engine.role_storage import require_guarded_role
            require_guarded_role(storage_lease)
            if storage_lease.lock_dir != lock:
                raise ValueError("role_guarded_storage_route_changed")
        _reference(meta)
        ...
        with _GUARD:
            ...
            owner = RoleLaunch(meta, lock_identity, lock_bytes, trigger_token, storage_lease)
            _OWNERS[id(owner)] = owner
        if storage_lease is not None:
            from orze.engine.role_storage import bind_guarded_role
            bind_guarded_role(storage_lease, owner)
        owner._write("INTENT")
