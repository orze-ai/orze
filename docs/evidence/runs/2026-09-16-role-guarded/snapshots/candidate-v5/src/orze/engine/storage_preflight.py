...
from orze.engine.gc_tree import GCRefused, identity, rename_no_replace_at
    ...
    try:
        ...
        for directory in (False, True):
            for collision in (False, True):
                ...
                probe.check()
                try:
                    rename_no_replace_at(parent, 'source', parent, 'target')
...
def role_storage_mode(role_cfg):
    mode = role_cfg.get('storage_mode', 'atomic')
    if type(mode) is not str or mode not in {'atomic', 'guarded'}:
        raise StoragePreflightError('storage_role_mode_invalid')
    return mode


def require_role_storage_at(root, role_cfg):
    """Recheck the chosen protocol on the actual captured role-lock route."""
    if role_storage_mode(role_cfg) == 'guarded':
        require_guarded_storage(root)
    else:
        require_atomic_rename_support(root)


def gc_storage_mode(cfg):
    gc = cfg.get('gc') or {}
    if not isinstance(gc, dict):
        raise StoragePreflightError('storage_gc_invalid')
    mode = gc.get('storage_mode', 'atomic')
    if type(mode) is not str or mode not in {'atomic', 'guarded'}:
        raise StoragePreflightError('storage_gc_mode_invalid')
    return mode


def require_guarded_storage(root):
    """Probe exclusive creation, collision preservation, sync and removal."""
    root, probe, fd = Path(root), None, None
    try:
        captured = _directories(root)
        fd = _open_directory(root)
        _verify_route(captured, fd)
        ...
        probe.create()
        for directory in (False, True):
            name = 'directory' if directory else 'file'
            probe.entry(probe.fd, name, directory=directory, raw=b'guarded-probe')
            try:
                probe.entry(probe.fd, name, directory=directory, raw=b'collision')
            ...
            probe.check()
            if not directory:
                read_fd = os.open(name, os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK, dir_fd=probe.fd)
                try:
                    if os.read(read_fd, 64) != b'guarded-probe':
                        raise StoragePreflightError('storage_probe_readback_unconfirmed', probe=probe.path)
                finally:
                    os.close(read_fd)
        _verify_route(captured, fd)
        ...
        raise StoragePreflightError('storage_guarded_probe_unconfirmed', path=root,
                                    probe=probe.path if probe else None) from exc
    finally:
        if probe is not None:
            probe.close()
        if fd is not None:
...
def require_gc_storage(root, cfg):
    if gc_storage_mode(cfg) == 'guarded':
        require_guarded_storage(root)
    else:
        require_atomic_rename_support(root)


def _ensure_route(root, *, gc_config=None):
    ...
    if gc_config is None:
        ...
        require_gc_storage(root, gc_config)
...
def require_role_storage(cfg, role_cfg):
    ...
    root = Path(control).absolute() / 'locks'
    role_storage_mode(role_cfg)
    try:
        fd = _open_directory(root, create=True)
        os.close(fd)
    except Exception as exc:
        raise StoragePreflightError('storage_route_unconfirmed', path=root) from exc
    require_role_storage_at(root, role_cfg)
...
def require_deployment_storage(cfg, results_dir):
    ...
    # Mixed per-role protocols must all be admitted; the first role alone
    # cannot establish that a later default-atomic role can release its lock.
    modes = set()
    for role in roles.values():
        if role_uses_atomic_rename(role):
            mode = role_storage_mode(role)
    ...
    if gc.get('enabled') and gc.get('checkpoints_dir'):
        ...
        _ensure_route(scope.checkpoints_dir, gc_config=cfg)
