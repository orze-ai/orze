...
def _closed(closure, binding, return_code):
    _require(type(return_code) is int and type(closure) is dict
             ...
             and type(closure.get('worker_returncode')) is int and closure['worker_returncode'] == return_code
...
def _clocks(record, attempts, *, cost_only=False):
    ...
    for call_index, call in enumerate(calls):
        ...
        _require(call['exit_code'] == 0 or cost_only, 'controller did not complete')
        _closed(supervision['closure'], binding, call['exit_code'])
        birth = (binding['worker']['pid'], binding['worker']['start_ticks'])
        _require(all(number(v, integer=True) and v > 0 for v in birth) and birth not in births,
                 'invalid or duplicate controller invocation')
        births.add(birth)
    ...
    return wall, terminal_order


def audit_scheduling(record, *, instance, protocol, expected_attempt_refs, selected_ref,
                     confirmation_ref=None, worker_command=None, _cost_only=False):
...
"""
    try:
        ...
        _require(type(_cost_only) is bool, 'invalid cost audit mode')
        raw = json.dumps({'record': captured, 'instance': instance, 'protocol': protocol,
            ...
            'command': worker_command, 'cost_only': _cost_only},
...
def _audit(inputs):
    ...
    cost_only = inputs['cost_only']
    _require(type(expected) is list and (0 if cost_only else 1) <= len(expected) <= MAX_ATTEMPTS,
             'invalid expected attempt inventory')
    expected = [digest(_ref(row)) for row in expected]
    _require(len(expected) == len(set(expected)), 'duplicate expected attempt')
    if cost_only:
        _require(inputs['selected'] is None and inputs['confirmation'] is None, 'cost audit cannot certify selection')
    selected = None if cost_only else digest(_ref(inputs['selected']))
    confirmation = digest(_ref(inputs['confirmation'])) if inputs['confirmation'] is not None else None
    _require((cost_only or selected in expected) and (confirmation is None or confirmation in expected and confirmation != selected),
    ...
    by_producer, by_evaluator = defaultdict(list), defaultdict(list)
    for row in _mapping(db['research_artifacts'], 'artifact_id', 'artifact').values():
        ...
        by_producer[digest(artifact['producer'])].append(artifact)
    for row in _mapping(db['research_observations'], 'observation_id', 'observation').values():
        ...
        by_evaluator[digest(observation['evaluator'])].append(observation)
    ...
    wall, terminal_order = _clocks(record, attempts, cost_only=cost_only)
    ...
    for key, attempt in attempts.items():
        ...
        request = parse_domain_task(idea['config'])
        _require(native['domain_kind'] == 'schedule_holdout'
        ...
        for item in native['source_snapshot']['inputs']:
            ...
            producer_key = digest(source['producer'])
            _require(producer_key in attempts and terminal_order[producer_key] < terminal_order[key],
                     'source producer did not close before evaluator invocation')
            producer_terminal = read_json(attempts[producer_key]['terminal_json'])
            _require(producer_terminal['outcome'] == 'completed'
                     and item['effect_sha256'] == producer_terminal['effect_receipt_sha256'],
                     'source effect differs from completed producer')
        ...
        produced = by_producer[key]
        observed = by_evaluator[key]
    ...
    coverage = {name:Counter({'valid':0,'invalid':0,'unknown':0}) for name in domain.PROTOCOLS}
    for value in evaluations.values():coverage[value['protocol']][value['verdict']['status']] += 1
    measurement = {'status':'completed' if valid else 'failed', 'metrics':metrics,
        ...
        'observations':dict(coverage[protocol]),'native_outcomes':dict(outcomes)}
    return {'schema':1,'scope':('frozen_scheduling_closed_native_costs' if cost_only
                              else 'frozen_scheduling_artifacts_and_declared_native_attempts'),
        'new_research_evidence':False,'campaign_identity_verified':False,
        'confirmation_scope':('none_closed_ledger_only' if cost_only else 'independent_evaluator_attempt_same_candidate'),
        'expected_attempt_inventory_sha256':digest(inputs['expected']), 'measurement':measurement,
        'evaluations':list(evaluations.values()),'observations_by_protocol':{k:dict(v) for k,v in coverage.items()},
        'evaluator_attempts_without_observation':analysis_actions-len(evaluations),
        'evaluator_sha256':_sha(Path(domain.__file__).read_bytes()),
        'evaluator_identity_sha256':evaluator_identity(protocol)}


def audit_scheduling_cost(record, *, instance, protocol, expected_attempt_refs, worker_command=None):
    """Account for a closed failed/empty campaign without asserting valid quality.

Nonzero controller exits are retained only with matching complete closure.
Every native attempt must still have a verified terminal, settlement, sources
and publications. Unfinished or corrupted native ledgers remain unavailable.
"""
    return audit_scheduling(record, instance=instance, protocol=protocol,
        expected_attempt_refs=expected_attempt_refs, selected_ref=None,
        worker_command=worker_command, _cost_only=True)


def evaluator_identity(protocol):
    """Identity of the task evaluator code and the chosen semantics."""
    _require(protocol in domain.PROTOCOLS, 'unsupported evaluator protocol')
...
This only verifies task data, evaluator and direction. A complete adapter must
also verify the campaign's arm, model, tools, initial state and full resource
