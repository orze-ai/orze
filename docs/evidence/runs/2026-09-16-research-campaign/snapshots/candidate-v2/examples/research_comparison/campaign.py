...
def describe():
    ...
    return _copy({'python': sys.executable, 'runtime': runtime, 'artifact_sha256': digest(artifact),
                  'environment': environment, 'verifier_sha256': digest(sources)})
...
def _bind(plan, run_id, inputs, runtime):
    ...
    if (any(digest(inputs[k]) != value for k, value in plan['shared'].items())
            ...
            or runtime['verifier_sha256'] != plan['verifier_sha256']):
...
def verify(record, task, arm, *, plan):
    ...
    raw = {}
    for name in ('request.json', 'worker.json', 'capture.json'):
        raw[name] = read_capture(folder / name, record['files'][name]['sha256'])
    if raw['request.json'] != request or record['error'] is not None or record['exit_code'] != 0:
        raise ValueError('campaign did not finish with a complete capture')
    ...
    if (worker['request_sha256'] != digest(request) or worker['before'] != request['runtime']
            or worker['after'] != worker['before'] or worker['status'] != 'completed'
            or worker['capture_sha256'] != record['files']['capture.json']['sha256']):
    ...
    result = verify_workload(raw['capture.json'], task, request=request)
