...
from .scheduling import audit_scheduling, evaluator_identity, verify_scheduling
...
def run(request, output):
    ...
    try:
        ...
        for cycle in range(1, tools['rounds'] + 1):
            ...
            command = [sys.executable, '-m', 'orze_pro.agents.research', '-c', str(config),
                '--backend', model['backend'], '--model', model['model'], '--endpoint', model['endpoint'],
                '--cycle', str(cycle), '--num-ideas', str(tools['num_ideas']), '--rules-file', str(rules),
                '--rules-sha256', rule_sha, '--lake-db', str(root / 'lake.db')]
...
def verify(capture, task, *, request):
    ...
    if [r['cycle'] for r in research] != list(range(1, request['inputs']['tools']['rounds'] + 1)):
    ...
    if (usage_scope['requests'] != expected_requests or len(usage_scope['journals']) != 1
            or usage_scope['journals'][0]['path'] != str(root / 'usage.jsonl')):
