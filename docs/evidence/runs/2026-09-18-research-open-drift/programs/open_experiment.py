...
def check_source(source, entry):
    ...
    io_names = {"load", "save", "loadtxt", "savetxt", "fromfile", "tofile",
                "memmap", "dump", "dumps", "load_library", "ctypes", "os",
                "sys", "subprocess"}
    for node in ast.walk(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            names = ([a.name for a in node.names] if isinstance(node, ast.Import)
                     else [node.module or ""])
            if any(n.split(".")[0] not in {"numpy", "sklearn", "scipy", "math"}
                   for n in names) or getattr(node, "level", 0):
                raise ValueError("only numpy, sklearn, scipy and math imports supported")
