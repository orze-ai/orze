"""Retire the still-reserved materials study; retain every unknown dollar bound.

No new pricing assumptions: all known tokens retain the old $50/M ceiling.
All 28 invocations have known usage; historical unknown bounds remain unchanged.
"""
...
def main():
    original = read('research-materials-20260918/budget.json')
    delivered = read('research-materials-final-delivery-20260918.json')
    assert original['new_reserve_usd'] == 224
    ...
    assert delivered['new_unknown_cost'] == 0
    ...
    parent = read('research-parent-20260918/budget.json')
    assert parent['new_reserve_usd'] == 280 and parent['new_release_usd'] == 0
    same(parent['effective_reserved_usd'], D(str(original['effective_reserved_usd']))+D(280))
    assert parent['previous_delivery_sha256'] == PROOFS['research-materials-final-delivery-20260918.json']
    context = read('research-context-20260918/budget.json')
    assert context['new_release_usd'] == 0
    same(context['effective_reserved_usd'], D(str(parent['effective_reserved_usd']))+D(str(context['new_reserve_usd'])))
    ...
    parent_release = read('rsi-budget-reconciliation-20260923/budget-release.json')
    assert parent_release['retired_pool'] == 'research-parent-20260918'
    assert parent_release['released_pool_usd'] == 280
    same(parent_release['previous_effective_global_reserved_usd'], current)
    same(parent_release['effective_global_reserved_usd'], current-D(str(parent_release['released_unused_usd'])))
    current = D(str(parent_release['effective_global_reserved_usd']))
    previous_path = 'rsi-model-screen-20260923/budget.json'
    allocation = read(previous_path)
    assert allocation['new_pool_usd'] == 135
    assert allocation['previous_ledger_sha256'] == PROOFS['rsi-budget-reconciliation-20260923/budget-release.json']
    same(allocation['previous_effective_global_reserved_usd'], current)
    same(allocation['effective_global_reserved_usd'], current+D(135))
    current = D(str(allocation['effective_global_reserved_usd']))
    records=[];known_tokens=0;unknown=0;call_ids=set()
    for arm in ('opus-A','opus-B','fable-A','fable-B'):
        folder=ROOT/'research-materials-20260918'/arm
    ...
    assert len(records)==28 and unknown==0
    retained=sum(D(str(r['retained_upper_usd'])) for r in records)
    released=D(224)-retained;effective=current-released
    result={'created':time.time(),'retired_pool':'research-materials-20260918',
        'retired_pool_must_not_resume':True,'previous_ledger_path':str(ROOT/previous_path),
        'previous_ledger_sha256':PROOFS[previous_path],
        'previous_effective_global_reserved_usd':float(current),'released_pool_usd':224,
        'closed_known_calls':28,'known_tokens':known_tokens,'all_token_upper_usd_per_million':50,
        'unknown_invocations':unknown,'unknown_full_original_bound_retained_usd':0,
