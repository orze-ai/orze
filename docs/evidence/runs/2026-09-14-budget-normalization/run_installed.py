...
Arguments: REPO PACKAGE_ROOT OUTPUT. Staged source is later checked against fixed Git.
...
package_report = json.loads((PACKAGE / "build-install.json").read_bytes())
assert package_report["passed"]
source_manifest = json.loads((PACKAGE / "source-manifest.json").read_bytes())
WHEEL = Path(package_report["wheel"]).resolve()
EXPECTED_WHEEL = package_report["wheel_sha256"]
assert sha(WHEEL.read_bytes()) == EXPECTED_WHEEL
SITE = Path(package_report["installed_site"])
...
loaded, source_hashes, events, paths = {}, {}, [], []
...
def audit(label):
    ...
    for name, module in tuple(sys.modules.items()):
        ...
        if member not in source_hashes:
            source_hashes[member] = source_manifest["src/" + member]
        assert actual == source_hashes[member], (name, "staged source mismatch")
...
test_files = ["test_cpu_proposal_paging_product_review.py", "test_cpu_budget_scan.py",
              "test_cpu_budget_normalization.py", "test_cpu_evidence_paging_loop_review.py",
              "test_cpu_product_loop.py::test_cli_runs_real_cpu_action_without_training_or_gpu"]
...
report = {"schema": 1, "classification": "installed Core payload under external global pytest harness",
          ...
          "staged_source_manifest_sha256": sha((PACKAGE / "source-manifest.json").read_bytes()), "wheel": str(WHEEL), "wheel_sha256": EXPECTED_WHEEL,
          ...
          "limits": ["Not a pure isolated-venv interpreter; existing global pytest is the external test harness.",
                     "Every observed loaded orze.* implementation must match fresh installed wheel and staged source bytes (fixed Git checked separately after commit).",
