...
commands = [
    ...
    [str(python), "-I", "-B", "-m", "pip", "--isolated", "--disable-pip-version-check", "install", "--no-index",
     "--no-compile", "--find-links", str(deps), str(wheel), "--report", str(root / "installed.json")],
    [str(python), "-I", "-B", "-m", "pip", "check"],
    [str(python), "-I", "-B", "-m", "orze", "--help"],
]
records = []
for index, command in enumerate(commands):
    path = root / f"offline-{index}.log"
