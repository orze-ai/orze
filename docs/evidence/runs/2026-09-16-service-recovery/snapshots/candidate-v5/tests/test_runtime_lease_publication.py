...
if child==0:
 os.setsid()
 Path('escaped.tmp').write_text(json.dumps({'pid':os.getpid()}))
 Path('escaped.tmp').rename('escaped.json')
 until=time.monotonic()+15
...
def test_expiry_after_actual_registration_retains_prepared_effect_without_second_terminal(launched, monkeypatch):
    ...
    recovered = _fresh_recovery(case)
    # F does not try to settle a still-RUNNING attempt; retaining it is a
    # successful conservative classification, not an exception requirement.
    assert (recovered["held"] is False and recovered["result"]["settled"] == []
            and recovered["result"]["retained"] == [
                {"reservation_id": case["permit"]["reservation_id"], "reason": "running"}])
