...
@pytest.mark.parametrize('state', ['absent', 'disabled', 'enabled', 'absent-enabled', 'masked', 'active', 'incomplete'])
def test_source_manager_state_requires_retired_units(monkeypatch, tmp_path, state):
    ...
    def show(command, **kwargs):
        ...
        props = {'Id': command[3], 'LoadState': 'not-found' if state.startswith('absent') else 'loaded',
                 'ActiveState': 'active' if state == 'active' else 'inactive',
                 'UnitFileState': 'enabled' if state.endswith('enabled') else 'disabled', 'MainPID': '0', 'ControlGroup': ''}
        if state == 'masked':
            props.update(LoadState='masked', UnitFileState='masked')
        if state == 'incomplete':
            props.pop('LoadState')
...
def test_public_cli_routes_explicit_recovery_arguments(monkeypatch, install):
    from orze import cli
    from orze.service import recovery
    expected = ['--source-service-config', '/project/old.json', '--service-config', '/project/new.json',
                '--request-id', 'operator-recovery'] + (['--install'] if install else [])
    observed = []
    monkeypatch.setattr(recovery, 'main', lambda arguments: observed.append(arguments) or 75)
    monkeypatch.setattr(sys, 'argv', ['orze', 'service', 'recover', *expected])
    assert cli.main() == 75 and observed == [expected]
