...
def runtime(arm):
    ...
    assert {name[len('src/orze/'):]: value for name, value in source['files'].items()
            if name.startswith('src/orze/') and not (name.startswith('src/orze/admin/ui/') and not name.startswith('src/orze/admin/ui/dist/') and not name.endswith('.py'))} == files
...
def selection(record, arm):
    ...
    assert selected['action_signature'] == [['result', canonical_digest({"specification_schema": "orze.native_cpu_action.v1", "action": action})]]
    action['command'][0] = '<same-verified-python-binary>'
    return {**selected, 'action_signature': [['result', canonical_digest({"specification_schema": "orze.native_cpu_action.v1", "action": action})]]}
...
if __name__ == '__main__':
    if sys.argv[1] == 'describe':
        ...
        report = summarize(ROOT / 'formal-03')
        save(ROOT / 'formal-03/summary.json', report)
        print(json.dumps({'all_quality_passed': report['all_quality_passed'], 'runs': report['executed_cli'], 'groups': report['groups']}))
