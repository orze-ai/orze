...
def check_source(source, entry):
    ...
    io_names = {"load", "save", "loadtxt", "savetxt", "fromfile", "tofile",
                ...
                "sys", "subprocess"}
    local_methods = {method.name for cls in ast.walk(tree)
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Attribute):
            # Common exception diagnostics and source-defined helper methods
            # were blocking reviewed numerical programs. These narrow forms
            # do not grant access to imported objects' private internals.
            exception_name = (node.attr == "__name__"
                and isinstance(node.value, ast.Call)
                and isinstance(node.value.func, ast.Name)
                and node.value.func.id == "type"
                and len(node.value.args) == 1 and not node.value.keywords)
            local_helper = (node.attr in local_methods
                and not node.attr.startswith("__")
                and isinstance(node.value, ast.Name)
                and node.value.id in {"self", "cls"})
            if ((node.attr.startswith("_") and not (exception_name or local_helper))
                    or node.attr in io_names
