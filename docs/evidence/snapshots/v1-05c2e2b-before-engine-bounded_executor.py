...
def require_executor_scope_clear(cwd):
    ...
    try:
        ...
        witness = None
    with _SCOPE_LOCK:
        for slot in _SCOPES.values():
            if slot.witness == witness:
                raise BoundedExecutorHOLD("bounded_executor_scope_" + slot.state)
...
def _reserve(scope, token, witness):
    with _SCOPE_LOCK:
        if scope in _SCOPES:
            raise BoundedExecutorHOLD("bounded_executor_scope_" + _SCOPES[scope].state)
        for previous in _SCOPES.values():
            if previous.witness == witness:
                raise BoundedExecutorHOLD("bounded_executor_scope_" + previous.state)
...
def run_bounded_executor(cmd, *, timeout, env, cwd, prepare=None, before_start=None):
    ...
    process, streams, binding, confirmed_timeout = None, None, None, None
    ...
    try:
        ...
        if timed_out:
            ...
            confirmed_timeout = error
            raise error
        return result
    except BaseException as exc:
        if exc is confirmed_timeout:
            raise  # Only our own fully closed timeout gets the legacy exception.
        if prepare_entered:
            held = True
