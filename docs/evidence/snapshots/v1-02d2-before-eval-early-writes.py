...
CALLING SPEC:
    launch_eval(idea_id, gpu, results_dir, cfg) -> EvalProcess | None
        ...
        raises: RuntimeError('evaluation_termination_unconfirmed') when an old
                evaluation start is unclosed or launch cleanup is uncertain.
    ...
    check_active_evals(active_evals, results_dir, cfg) -> list[(idea_id, gpu)]
        ...
        raises: RuntimeError('evaluation_termination_unconfirmed') and retains
                the active handle when whole-process cleanup is not confirmed.
...
def launch_eval(idea_id: str, gpu: int, results_dir: Path,
    ...
    # This also gates the existing-output reconciliation below. After a
    # controller restart there may be no EvalProcess in memory, while an old
    ...
    try:
        if lake is not None:
            ...
            attempt_ref = begin(lake, idea_dir, attempt_id, gpu)
            stage_started = True
        bundle = None
...
def run_eval(idea_id: str, gpu: int, results_dir: Path, cfg: dict, lake=None):
    ...
    if getattr(ep, "attempt_ref", None) is not None:
        from orze.engine.native_evaluation import finish
        finish(lake, ep, results_dir / idea_id, cfg, ep.process.poll(),
