...
class ProductProcess:
    def __init__(self, root, mode):
        ...
        if mode == "role_loss":
            ...
            (root / ".orze" / "triggers").mkdir()
            (root / ".orze" / "triggers" / ("_trigger_" + ROLE)).write_text("Run this tiny CPU fixture once.\n")
        ...
        env = {"PATH": os.environ.get("PATH", ""), "PYTHONPATH": os.pathsep.join([str(CORE_SOURCE), str(PRO_SOURCE)]),
               "PYTHONDONTWRITEBYTECODE": "1", "CUDA_VISIBLE_DEVICES": "", "ORZE_PRO_SKIP_LICENSE_CHECK": "1"}
    ...
    def stop(self, timeout=8):
        ...
        cfg = load_project_config(str(self.root / "orze.yaml"))
        return stop_controller(cfg, timeout=timeout)
...
def test_real_controller_stop_requires_drained_tree_and_observed_exit(product_process, mode):
    from orze.engine.controller_session import CompletedControllerStop
