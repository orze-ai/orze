...
def native(shot, row, root):
    """Validate each captured terminal, not merely its worker's audit tag."""
    ref = {k: row[k] for k in ("task_id", "phase", "generation", "attempt_id")}
    ...
    for name in ("kind", "origin"):
        equal(binding[name], "native_cpu_action")
    ...
    closure(terminal["process_tree"], binding["supervision"], terminal["return_code"])
    ...
    tag = "first" if ref["task_id"] == "idea-first" else "second"
    events = [e for e in shot["worker_events"] if e["tag"] == tag]
    equal(len(events), 1)
    equal({k: events[0][k] for k in ("pid", "start_ticks")}, binding["supervision"]["worker"])
    base = folder + "/_execution_effects/" + ref["attempt_id"]
    prepared_entry = shot["files"][base + "/prepared.json"]
    prepared, committed = decode(prepared_entry["utf8"]), decode(shot["files"][base + "/committed.json"]["utf8"])
    ...
    equal(committed, {**identity, "event": "effect_committed", "prepared_sha256": prepared_entry["sha256"]})
    equal(terminal["effect_receipt_sha256"], prepared_entry["sha256"])
    equal(terminal["observation_ids"], [])
    artifacts = [a for a in shot["database"]["research_artifacts"]
                 if a["producer_attempt_id"] == ref["attempt_id"]]
    equal(len(artifacts), int(terminal["return_code"] == 0))
    equal(sorted(terminal["artifact_ids"]), sorted(a["artifact_id"] for a in artifacts))
    for artifact in artifacts:
        record = decode(artifact["record_json"])
        equal(record["producer"], ref)
        equal({k: artifact["producer_" + k] for k in ref}, ref)
        equal(record["artifact_id"], artifact["artifact_id"])
        equal(record["logical_name"], artifact["logical_name"])
        equal(record["spec_fingerprint"], binding["action_sha256"])
        equal(record["scope"], root + "/results")
        content = shot["files"][record["path"]]
        equal(record["content_sha256"], content["sha256"])
        equal(record["size_bytes"], content["bytes"])
        equal(decode(content["utf8"]), {"tag": tag})
...
def one(report, mode):
    ...
    native(before, original, report["root"])
    ...
    if mode == "baseline":
        ...
        for attempt in final["execution_attempts"]:
            native(after, attempt, report["root"])
        equal(final["research_observations"], [])
        ...
        equal(recovery[0]["scope"], report["root"] + "/results")
        equal(decode(recovery[0]["binding_json"]), crash["permit"]["budget_scope"])
        assert type(recovery[0]["nonce"]) is str and re.fullmatch(r"[0-9a-f]{48}", recovery[0]["nonce"])
        summary = decode(recovery[0]["summary_json"])
        equal(sorted(summary), ["already_settled", "examined", "retained", "schema", "settled"])
        equal(summary["schema"], 1)
        equal(summary["retained"], [])
