...
def run_one(root, domain, variant, arm):
...
def main():
    ...
    scoped = ["examples/acceptance", "examples/research_efficiency"]
    paths = subprocess.check_output(["git", "ls-files", "-z", "--", *scoped],
                                    cwd=APPLICATION).decode().split("\0")
    source_before = {p: hashlib.sha256((APPLICATION / p).read_bytes()).hexdigest() for p in paths if p}
    source_before.update({str(p): hashlib.sha256(p.read_bytes()).hexdigest()
    ...
    for ordinal, spec in enumerate(schedule):
        ...
        record, pointer = run_one(folder, spec["domain"], spec["variant"], spec["arm"])
    ...
    source_after = {p: hashlib.sha256((Path(p) if Path(p).is_absolute() else APPLICATION / p).read_bytes()).hexdigest()
