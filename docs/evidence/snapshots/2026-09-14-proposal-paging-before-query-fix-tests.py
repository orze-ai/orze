...
def test_shared_revision_rejects_sql_change_even_if_records_are_equal(catalog, mutation):
    ...
    if mutation == 'peer_commit':
        # SQLite can elide value=value without changing data_version. Commit
        # genuinely different, still-valid metadata, then restore exact bytes.
        original = store.get_request(lake.conn, str(catalog[1]), 'r-000')
        changed = store.seal_record({**original, 'created_at': original['created_at'] + ' changed'})
        revision = lake.conn.execute('PRAGMA data_version').fetchone()[0]
        with sqlite3.connect(lake.db_path) as peer:
            peer.execute("UPDATE cpu_proposal_requests SET record_json=? WHERE request_id='r-000'",
                         (store.canonical(changed),))
            peer.commit()
            peer.execute("UPDATE cpu_proposal_requests SET record_json=? WHERE request_id='r-000'",
                         (store.canonical(original),))
            peer.commit()
        assert lake.conn.execute('PRAGMA data_version').fetchone()[0] != revision
    else:
        lake.conn.execute('BEGIN IMMEDIATE')
...
def test_selected_store_page_uses_one_bounded_row_query(catalog):
    populate(catalog, 32)
    lake, results, _ = catalog
    statements = []
    lake.conn.set_trace_callback(statements.append)
    try:
        page = store.records_page(lake.conn, str(results), request_ids=[f'r-{i:03}' for i in range(32)])
    finally:
        lake.conn.set_trace_callback(None)
    assert len(page['requests']) == 32
    assert len([sql for sql in statements if 'FROM main.cpu_proposal_requests' in sql]) == 1
