"""V1-02C design diagnostics, intentionally outside the Core test tree.

These exercise existing public phase/storage boundaries, not a proposed API.
Injection seams document the old interleaving and must remain observable when
the final production admission API is chosen. No provider or GPU is started.
"""
...
def test_successful_append_after_parse_is_not_erased_without_admission(engine, monkeypatch):
    ...
    assert appended == [True], "old interleaving must actually append under the real lock"
