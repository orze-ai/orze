...
def _cached(row, folder):
    """Read exact confirmed historical publication, never synthesize OS proof."""
...
def run_native_pre_script(idea_id, gpu, results_dir, cfg, lake, cmd, timeout, env):
    ...
    try:
        ...
        with execution_transaction(lake, folder) as tx:
            ...
            terminal.update(effect_receipt_sha256=digest, lifecycle_phase="training",
                            lifecycle=lifecycle_fence(lake, idea_id, "training"))
