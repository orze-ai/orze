...
def _configuration(path, cfg, kind):
    ...
    import yaml
    from orze.engine.artifact_publication import _identity, _path_identities, _verify_identities
    value = {}
...
def launch(idea_id, gpu, results_dir, cfg, *, kind, idea_cfg_path, lake):
    ...
    verify_preflight_source(lake, folder, cfg, preflight_capture)
    ...
    try:
        with sealed_payload(raw) as payload_fd:
            tp.attempt_ref = attempts.begin(lake, tp, folder,
                launch_inputs=inputs, artifact_binding=bound, cfg=cfg)
