...
No provider, process launch, lifecycle writes, observation ranking or scientific
validation occurs here. Worker-declared validity is recorded, not promoted.
...
def _read(path, maximum):
    ...
    try:
        ...
        while True:
            ...
            total += len(chunk)
            if total > maximum:
                raise AttemptEffectBusy("observation_file_size_limit")
            chunks.append(chunk)
        if files._identity(os.fstat(fd)) != identities[-1][2]:
            raise AttemptEffectBusy("observation_file_changed")
