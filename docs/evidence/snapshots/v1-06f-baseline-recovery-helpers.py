...
def run_cli(project, label, *, crash=False, expected=0):
    ...
    command = [sys.executable, "-m", "cpu_terminal_recovery_helpers"]
    if crash:
...
def new_crashed_project(root, *, exit_code=0):
    root.mkdir()
...
def make_crashed_project(tmp_path, request):
    ...
    def make(*, exit_code=0):
        project = new_crashed_project(tmp_path / ("project-" + str(len(projects))), exit_code=exit_code)
