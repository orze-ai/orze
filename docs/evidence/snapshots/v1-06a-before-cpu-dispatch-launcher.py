...
def launch(idea_id: str, gpu: int, results_dir: Path, cfg: dict, lake=None) -> TrainingProcess:
    ...
    if lake is not None:
        ...
        require_preflight_history_closed(lake, results_dir / idea_id, cfg)
        preflight_capture = None
    ...
    if lake is not None:
        verify_preflight_source(lake, results_dir / idea_id, cfg, preflight_capture)
    elif not verify_artifact_preflight_receipt(
            idea_id, results_dir, cfg):
        raise LaunchIntegrityError(
            "artifact_preflight_receipt_missing_or_stale")
    ...
    tp = SimpleNamespace(
        ...
        artifact_preflight_capture=preflight_capture,
    )
    if lake is not None:
        ...
        try:
            ...
            if (get_artifact_contract(cfg) is None and replication is None
                    and getattr(preflight_capture, "source", None) is None):
    ...
    try:
        ...
        if getattr(tp, "attempt_ref", None) is not None:
            ...
            verify_preflight_source(lake, results_dir / idea_id, cfg,
                                    preflight_capture, consumer_ref=tp.attempt_ref)
            _assert_controller_runtime_attested(cfg)
            _assert_launch_authorized(idea_id, results_dir, cfg)
            _assert_gpu_authorized(gpu, cfg)
            _assert_campaign_evidence_authorized(cfg, lake)
...
def _terminate_training(tp: TrainingProcess, results_dir: Path, cfg: dict,
                        reason: str) -> bool:
    """Publish interruption only after whole-execution termination is proven."""
    phase = "posthoc" if getattr(tp, "is_posthoc", False) else "training"
    try:
        return_code = terminate_execution(
            tp, results_dir / tp.idea_id, phase=phase,
            reaper=_terminate_and_reap,
        )
    except TerminationUnconfirmed:
        logger.error("[TERMINATION-HOLD] %s: %s", tp.idea_id, reason)
        return False
    tp.close_log()
    try:
        write_interruption_receipt(
            tp, results_dir, cfg, reason=reason,
            terminating_signal="SIGTERM", return_code=return_code,
        )
    except Exception as exc:
        logger.warning(
    ...
    for gpu in list(active.keys()):
        ...
        try:
            if getattr(tp, "_termination_unconfirmed", False) is True:
                raise TerminationUnconfirmed("training_termination_unconfirmed")
            require_no_unconfirmed_stop(results_dir / tp.idea_id)
        except TerminationUnconfirmed:
            tp._termination_unconfirmed = True
            logger.error("[TERMINATION-HOLD] %s remains active", tp.idea_id)
            continue
        ...
        if ret is None:
            ...
            if elapsed > tp.timeout:
                ...
                if not _terminate_training(tp, results_dir, cfg, "timeout"):
                    continue
                error_msg = "Timed out"
            ...
            if check_stalled(tp, stall_minutes):
                ...
                if not _terminate_training(tp, results_dir, cfg, "stall"):
                    continue
            ...
            if ret is None and elapsed > 120:
                ...
                if is_zombie:
                    ...
                    if not _terminate_training(tp, results_dir, cfg, "zombie"):
                        continue
            ...
            if ret is None and _watchdog_check(tp):
                ...
                if not _terminate_training(tp, results_dir, cfg, "watchdog"):
                    continue
            ...
            if fatal and tp.process.poll() is None:
                ...
                if not _terminate_training(tp, results_dir, cfg, "fatal_log"):
                    continue
            ...
            if kill_file.exists():
                ...
                if not _terminate_training(tp, results_dir, cfg, "admin_kill"):
                    continue
