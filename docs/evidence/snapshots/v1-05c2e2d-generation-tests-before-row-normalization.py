...
assert control.current_instance(lake.conn,scope)==row(ctx)
...
assert lake.conn.execute('SELECT current_id,generation,pending_grant FROM controller_scope_heads').fetchone()==(ctx.controller_id,0,None)
