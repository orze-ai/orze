...
class QueuePolicy:
    ...
    def decide(self, snapshot, budget):
        ...
        if not snapshot["active"] and self.declaration["idle"] == "stop":
            return {"kind": "Stop", "reason": "queue_drained", "wakeup": None}
        return {"kind": "Wait", "reason": "actions_running" if snapshot["active"] else "queue_empty",
                "wakeup": now + self.declaration["wait_seconds"]}
...
def iteration(engine):
    ...
    if not engine._health_monitor.check_before_write():
        engine._stop_event.wait(engine._health_monitor.retry_delay)
        return True
    ...
    for queued in queue:
        ...
        if type(parsed) is not dict:
            raise CPUExecutionError("execution: action configuration is not a mapping")
