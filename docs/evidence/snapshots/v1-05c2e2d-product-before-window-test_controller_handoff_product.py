...
if 'orze.cli' in getattr(sys,'orig_argv',()):
    ...
    # Preserve the actual Popen and protocol; redirect only private diagnostics.
    import subprocess
    ...
    child_fault=os.environ.get('ORZE_HANDOFF_FIXTURE_CHILD_FAULT','')
    if channel is not None and child_fault:
        from orze.engine import controller_handoff as handoff
        if child_fault=='close_lease_after_commit':
            actual_receive=handoff._receive
            def receive(channel,peer_pid,deadline):
                packet=actual_receive(channel,peer_pid,deadline)
                if packet['event']=='COMMIT':
                    lease=gpu_lease._registered[0]
                    descriptor=lease.fd
                    lease.close()
                    (root/'child-fault.json').write_text(json.dumps({
                        'fault':child_fault,'event':'COMMIT','fd':descriptor,
                        'closed':lease._closed,'after_real_receive':True}))
                return packet
            handoff._receive=receive
        elif child_fault=='replace_source_ack_in_consume':
            actual_consume=handoff._Admission.consume
            def consume(self,conn,ctx):
                raw=conn.execute('SELECT ack_json FROM main.controller_sessions '
                    'WHERE controller_id=?',(self.source_controller_id,)).fetchone()[0]
                ack=json.loads(raw)
                ack['binding_sha256']='0'*64 if ack['binding_sha256']!='0'*64 else '1'*64
                conn.execute('UPDATE main.controller_sessions SET ack_json=? '
                    'WHERE controller_id=?',(json.dumps(ack,sort_keys=True,separators=(',',':')),
                    self.source_controller_id))
                (root/'child-fault.json').write_text(json.dumps({
                    'fault':child_fault,'same_transaction':conn.in_transaction,
                    'source_controller_id':self.source_controller_id}))
                return actual_consume(self,conn,ctx)
            handoff._Admission.consume=consume
        else:
            raise RuntimeError('unknown fixture child fault')
    def idle(gpus):
        assert list(gpus)==[0]
...
def logged_popen(command,*args,**kwargs):
    if list(command[:3])==[sys.executable,'-m','orze.cli']:
        with (Path(sys.argv[1]).parent/'successor.log').open('ab') as log:
            kwargs['stdout']=log;kwargs['stderr']=subprocess.STDOUT
            return actual_popen(command,*args,**kwargs)
    return actual_popen(command,*args,**kwargs)
subprocess.Popen=logged_popen
...
if fault:
    ...
    def receive(channel,peer_pid,deadline):
        ...
        return value
    handoff._receive=receive
if fault=='boolean_commit_schema':
    actual_send=handoff._send
    def send(channel,value,*args,**kwargs):
        if value['event']=='COMMIT':
            value={**value,'schema':True}
            result=actual_send(channel,value,*args,**kwargs)
            (Path(sys.argv[1]).parent/'observer-fault.json').write_text(json.dumps(
                {'fault':fault,'event':'COMMIT','schema':True,'after_real_send':True}))
            return result
        return actual_send(channel,value,*args,**kwargs)
    handoff._send=send
try:
...
class HandoffProcess(ProductProcess):
    ...
    def __init__(self, root, mode):
        ...
        self._old_channel_closed = False
    ...
    def pump(self):
        ...
        if _alive(self.controller_fd) and not self._old_channel_closed:
        ...
        for channel in select.select(choices, [], [], 0.02)[0]:
            if channel is self.channel:
                ...
                value['predecessor_alive_at_birth'] = _alive(
                    getattr(self, 'predecessor_fd', self.controller_fd))
    ...
    def restart(self, request_id, *, cli=False, timeout=12, fault='', child_fault=''):
        if cli:
        ...
        if child_fault:
            environment['ORZE_HANDOFF_FIXTURE_CHILD_FAULT'] = child_fault
...
def test_second_actual_handoff_retains_three_generations_and_two_grants(handoff_process):
    controller = _prepare_old(handoff_process, 'normal')
    first = controller.result(controller.restart('generation-one'))
    assert first['completed'] is True
    first_grant = _assert_started(controller, 'generation-one')
    prior = controller.rows('controller_instances')
    assert len(controller.births) == 1
    predecessor = controller.births[0]['pid']
    controller.predecessor_fd = controller.pidfds[predecessor]
    assert _alive(controller.predecessor_fd)
    second = controller.result(controller.restart('generation-two'))
    assert second['completed'] is True
    assert len(controller.births) == 2
    born = controller.births[1]
    assert born['predecessor_alive_at_birth'] is False
    assert not _alive(controller.predecessor_fd)
    assert _alive(controller.pidfds[born['pid']])
    instances = sorted(controller.rows('controller_instances'), key=lambda row: row['generation'])
    assert [row['generation'] for row in instances] == [0, 1, 2]
    assert [(row['controller_id'], row['identity_json']) for row in instances[:2]] == [
        (row['controller_id'], row['identity_json']) for row in sorted(prior, key=lambda row: row['generation'])]
    assert instances[2]['predecessor'] == instances[1]['controller_id']
    assert controller.rows('controller_scope_heads')[0]['current_id'] == instances[2]['controller_id']
    grants = controller.rows('controller_handoffs')
    assert len(grants) == 2 and all(row['state'] == 'STARTED' for row in grants)
    assert next(row for row in grants if row['request_id'] == 'generation-one') == first_grant
    assert next(row for row in grants if row['request_id'] == 'generation-two')['source_controller_id'] == instances[1]['controller_id']
    sessions = controller.rows('controller_sessions')
    assert len(sessions) == 3
    assert all(next(row for row in sessions if row['controller_id'] == identity['controller_id'])['ack_json']
               for identity in instances[:2])
    assert next(row for row in sessions if row['controller_id'] == instances[2]['controller_id'])['ack_json'] is None
    controller.assert_successor_holds_real_lease()


def test_actual_commit_cannot_start_after_owned_gpu_lease_closed(handoff_process):
    controller = _prepare_old(handoff_process, 'normal')
    result = controller.result(controller.restart(
        'commit-resource', child_fault='close_lease_after_commit'))
    fault = json.loads((controller.root / 'child-fault.json').read_text())
    assert fault['fault'] == 'close_lease_after_commit' and fault['event'] == 'COMMIT'
    assert fault['after_real_receive'] is True and fault['closed'] is True
    assert result['completed'] is False
    grants = controller.rows('controller_handoffs')
    assert len(grants) == 1 and grants[0]['state'] != 'STARTED'
    assert not any(event['event'] in {'successor_gpu_idle_spy', 'successor_worker_ready'}
                   for event in controller.events)


def test_actual_consume_validates_source_ack_in_its_own_writer_transaction(handoff_process):
    controller = _prepare_old(handoff_process, 'normal')
    result = controller.result(controller.restart(
        'consume-source', child_fault='replace_source_ack_in_consume'))
    fault = json.loads((controller.root / 'child-fault.json').read_text())
    assert fault['fault'] == 'replace_source_ack_in_consume'
    assert fault['same_transaction'] is True
    assert result['completed'] is False
    grants = controller.rows('controller_handoffs')
    assert len(grants) == 1 and grants[0]['state'] != 'STARTED'
    assert not any(event['event'] in {'successor_gpu_idle_spy', 'successor_worker_ready'}
...
def test_actual_commit_wire_requires_integer_schema_not_boolean(handoff_process):
    controller = _prepare_old(handoff_process, 'normal')
    result = controller.result(controller.restart(
        'commit-schema', fault='boolean_commit_schema'))
    fault = json.loads((controller.root / 'observer-fault.json').read_text())
    assert fault == {'fault': 'boolean_commit_schema', 'event': 'COMMIT', 'schema': True, 'after_real_send': True}
    assert result['completed'] is False
    assert len(controller.births) == 1
    grants = controller.rows('controller_handoffs')
    assert len(grants) == 1 and grants[0]['state'] != 'STARTED'
