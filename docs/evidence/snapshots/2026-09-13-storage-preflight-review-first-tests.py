...
from orze.engine import gc_safety, storage_preflight as storage
...
def test_path_replacement_with_same_probe_names_cannot_move_foreign_source(tmp_path, monkeypatch):
    ...
    def swap_before_real_rename(source, target):
    ...
    monkeypatch.setattr(storage, "rename_no_replace", swap_before_real_rename)
...
def test_cleanup_retains_untracked_entry_inside_original_probe(tmp_path, monkeypatch):
    ...
    def insert_after_real_rename(source, target):
        result = actual(source, target)
        if not foreign:
    ...
    monkeypatch.setattr(storage, "rename_no_replace", insert_after_real_rename)
