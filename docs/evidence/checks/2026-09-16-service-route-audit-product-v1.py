"""Actual CPU controllers survive service route drift; manager actions are forbidden."""
import copy
import hashlib
...
def main(root):
    ...
    report={'script_sha256':sha(Path(__file__).read_bytes()),'helper_sha256':sha(HELPER.read_bytes()),
        'baseline_runtime_sha256':sha(BASE.read_bytes()),'cases':[],
        'limits':['Real Core CLI controllers and CPU worker output, manager install operations replaced.',
                  ...
                  'Current route audit only; no durable owner identity, real manager environment, scoped service or deployment guarantee.']}
    env=dict(os.environ,PYTHONDONTWRITEBYTECODE='1',CUDA_VISIBLE_DEVICES='',PYTHONPATH=str(CORE/'src'))
    for mode in ('config-drift', 'dotenv-drift', 'matching'):
...
""" % (str(marker),str(release),json.dumps(expected))
        ...
        key='ORZE_ROUTE_PRODUCT_RESULTS'
        assert key not in os.environ
        if mode=='dotenv-drift':
            cfg['results_dir']='${'+key+'}'
            (project/'.env').write_text(key+'='+str(results)+'\n')
        config.write_text(yaml.safe_dump(cfg))
        initial_config=config.read_bytes();initial_env=(project/'.env').read_bytes()
        ...
        with stdout.open('xb') as out,stderr.open('xb') as err:
            ...
            try:
                ...
                with patch.object(watchdog,'_read_pid',read_pid),patch.object(watchdog,'_is_orze_running',forbidden),patch.object(watchdog.os,'kill',own_probe),patch.object(watchdog.os,'killpg',forbidden),patch.object(watchdog.subprocess,'run',forbidden),patch.object(watchdog.subprocess,'Popen',forbidden),patch.object(watchdog,'_notify_failure_loop',lambda *args:None):
                    target=project/'different-results';target.mkdir()
                    (target/'.orze_stop_all').write_text('operator stop at current configured route')
                    if mode=='config-drift':
                        changed_cfg=dict(cfg,results_dir=str(target));config.write_text(yaml.safe_dump(changed_cfg))
                    elif mode=='dotenv-drift':
                        (project/'.env').write_text(key+'='+str(target)+'\n')
                    save(case/'route-inputs.json',{'config_before':initial_config.decode(),'config_at_audit':config.read_text(),
                        'dotenv_before':initial_env.decode(),'dotenv_at_audit':(project/'.env').read_text(),'target_stop':str(target/'.orze_stop_all')})
                ...
                config.write_bytes(initial_config);(project/'.env').write_bytes(initial_env)
        ...
        if mode!='matching':assert caught=='runtime_contract_rejected' and events==[], 'runtime drift was not rejected before controller inspection'
