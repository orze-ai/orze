"""Alternate baseline and current complete ingress with the same normal admission.

Only ingress preparation dispatch changes. Public lookup and writer code are
identical. Timing excludes setup, separate counters, and memory profiling.
"""
import hashlib
...
def run(root):
    ...
    old = b.load("orze.engine._preparation_baseline", OUT / "baseline/idea_ingress.py")
    arms = {"old": old.ingest_ideas_source, "new": b.idea_ingress.ingest_ideas_source}
    inputs = [REPO / "src/orze/core/proposal_admission.py", REPO / "src/orze/idea_lake.py",
              REPO / "src/orze/engine/idea_ingress.py", OUT / "baseline/idea_ingress.py",
              REPO / "docs/evidence/checks/2026-09-15-ingress-benchmark.py"]
    fingerprints = {str(p): b.sha(p.read_bytes()) for p in inputs}
    report = {"baseline": "1b8ac7b5e7a398ceae95f58bbd47e8b3ae0ab895", "sources": fingerprints,
              ...
              "limits": ["Synthetic admitted metadata in four mixed-case statuses; no executed history or scientific quality comparison.",
                         ...
                         "Semantically equal YAML still parses; crowded histories retain capacity refusal. Sparse controls retain extra costs.",
                         "Both arms run complete current writers and source checks; old ingress alone uses the unchanged public lookup.",
        ...
        try:
            ...
            with patch.object(b.idea_ingress.logger, "warning", lambda *a, **k: None):
                ...
                for name, entry in arms.items():
                    ...
                    admission = current.admit_proposal
                    def outcome(*args, **kwargs):
                        value = admission(*args, **kwargs)
                    ...
                    try:
                        with patch.object(current.yaml, "safe_load", counted), patch.object(current, "admit_proposal", outcome):
                            assert call(entry) == measured["value"]
                    finally:
                    ...
                    counters[name] = {"yaml_loads": len(calls),
                                      ...
                                      "owner_mapping_queries": [s for s in statements if s.startswith("SELECT config_hash, idea_id FROM ideas")]}
                expected_yaml = 384 if mode == "semantic" else 256
                ...
                assert len(counters["old"]["owner_mapping_queries"]) == 1
                assert counters["new"]["owner_mapping_queries"] == []
                assert counters["old"]["candidate_queries"] == counters["new"]["candidate_queries"]
            after = b.sha("\n".join(engine.lake.conn.iterdump()).encode())
            assert before == after
            assert source.read_text() == text
            report["rows"].append({"history": history, "mode": mode,
                                   "entry": "complete_ingress",
