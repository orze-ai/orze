"""Read-only C3b evidence verifier. Run from the Core repository root.
...
evidence = json.loads((root / "docs/evidence/2026-09-11-v1-05c3b-role-receipt-bounds.json").read_bytes())
...
assert evidence["schema"] == 1 and evidence["slice"] == "V1-05C3b"
assert evidence["whole_v1_complete"] is False
assert evidence["test_classification"] == {
    "preceding_commit_behavior_reds": 2, "controls": 1,
    "new_storage_mechanisms": 10, "draft_reds": 0, "total_new_cases": 13,
}
...
for snapshot in evidence["snapshots"]:
    ...
    if snapshot.get("commit") is not None:
        assert raw == git("core", "show", snapshot["commit"] + ":" + snapshot["source"])
    else:
        assert snapshot["origin"] == "first_two_case_test_before_regular_width_extension"
        assert (root / "tests/test_role_delivery_receipt_bounds.py").read_bytes().startswith(raw)
...
expected_new = {"tests/test_role_delivery_receipt_bounds.py", "tests/test_role_receipt_storage_boundaries.py"}
...
for key, expected in {"core": (3639, 7), "pro": (974, 0), "paired": (60, 0), "target": (13, 0)}.items():
