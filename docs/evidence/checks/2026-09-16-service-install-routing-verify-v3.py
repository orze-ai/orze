...
def archive_checks():
    ...
    with tarfile.open(partial,'r:gz') as archive:
        ...
        for member in members:
            ...
            if member.issym():assert original.is_symlink() and os.readlink(original)==member.linkname
            else:assert member.isfile() and archive.extractfile(member).read()==original.read_bytes()
...
def main():
    ...
    specs=[('baseline',0,(78,0,0,0)),('old-feature',1,(24,18,0,0)),('targeted-v1',0,(102,0,0,0)),
        ...
        ('product-v1',0,None),('archive-v1',1,None),('archive-v2',0,None),('verify-preflight',1,None),('verify-preflight-v2',1,None)]
    ...
    for name in ('probe-v1','product-old-v1','product-controls-v1','product-v1','archive-v1','archive-v2','verify-preflight','verify-preflight-v2'):
