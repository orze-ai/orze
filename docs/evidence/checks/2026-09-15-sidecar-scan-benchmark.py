...
def main(root, layout, repetitions, memory=False):
    ...
    report = {"baseline": "a952321b33e6973d6c580f5593139bebdce19e30", "source_files": before_sources,
              ...
              "limits": ["Synthetic completed metadata and conflicting source titles; no scientific outcome or worker.",
                         ...
                         "A single partially traversed file still requires fresh payload reading and prefix YAML parsing.",
                         "Only baseline/current prefix classes and their respective section parser change; ingress/writers identical.",
            ...
            with patch.object(idea_ingress.logger, "warning", lambda *a, **k: None):
                ...
                for name, module in modules.items():
                    ...
                    stats = {"sidecar_payload_reads": 0, "sidecar_payload_bytes": 0,
                             ...
                             "max_name_window": 0, "begins": 0, "rollbacks": 0}
                    def counted(path):
                        value = fresh(path)
                        if path.parent == side:
