...
def installed_check(maps, wheels):
    ...
    for observation in r["sys_path_observations"]:
        ...
        position=observation["sys_path"].index(r["installed_core_only_site"])
        for preceding in observation["sys_path"][:position]:
            assert preceding==str(C/"tests")
            assert not (Path(preceding)/"orze").exists() and not (Path(preceding)/"orze.py").exists()
