"""Alternate old/new repair on identical imported metadata; time and memory separately.
...
BASE = ROOT / "docs/evidence/runs/2026-09-15-identity-repair-staging/baseline"
spec = importlib.util.spec_from_file_location("old_repair_lake", BASE / "idea_lake.py")
old = importlib.util.module_from_spec(spec)
spec.loader.exec_module(old)
FUNCTIONS = {"old": old.IdeaLake._repair_admitted_config_hashes,
...
def main(root, mode):
    ...
    report = {"mode": mode, "source_files": sources, "script_sha256": sha(Path(__file__).read_bytes()),
              ...
              "rows": [], "limits": ["Imported metadata only, no scientific or worker throughput claim.",
                 "Memory is Python tracemalloc, not RSS; memory runs are excluded from latency results.",
    ...
    for count, payload in ((100, 1024), (1000, 1024), (5000, 1024), (500, 8192)):
        ...
        if mode == "time":
            ...
            for name in FUNCTIONS:
                ...
                try:
                    with patch.object(staging, "_apply", apply):
        ...
        print(json.dumps({"mode": mode, "records": count, "payload": payload,
            ...
            "memory": {k: a[0]["current_peak_python_bytes"] for k, a in row["arms"].items()}}), flush=True)
