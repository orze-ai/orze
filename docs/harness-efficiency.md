...
live owner or trainer. Every non-initial global state must have an immutable
transition ending in that state. The `ideas` and `idea_state` tables must also
contain the exact same idea-ID universe in both directions; a missing or orphan
current state is a contradiction, even when no transition refers to it. A
missing deployed global/stage history, redirected or
