...
def request_evaluation_retry(idea_id: str, results_dir: Path,
    ...
    try:
        with attempt_effect_lock(Path(results_dir) / idea_id) as lease:
            from orze.engine.claim_authority import require_closed_claim_attempts
            require_closed_claim_attempts(Path(results_dir) / idea_id,
                                          lake=lake, effect_lease=lease)
...
def _request_evaluation_retry(idea_id: str, results_dir: Path,
                              cfg: dict, lake) -> dict:
