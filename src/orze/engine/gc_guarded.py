...
def _copy_file(source, destination, expected, check, target_parent_identity):
    source_parent = target_parent = source_fd = target_fd = None
    try:
        source_parent = _open_directory(source.parent)
        target_parent = _open_directory(destination.parent)
        if identity(os.fstat(target_parent))[:3] != target_parent_identity:
            raise GCRefused('gc_guarded_copy_parent_changed')
...
def copy_new(tree, destination, check):
    ...
    try:
        if os.fstat(parent).st_dev != tree.root_identity[0]:
            raise GCRefused('gc_cross_device_refused')
        parent_identity = identity(os.fstat(parent))[:3]
    finally:
        os.close(parent)
    digests, created = {}, {}
    for relative, expected in tree.entries:
        ...
        for ancestor in ancestors:
            pinned = (parent_identity if ancestor == destination.parent else
                      created['.' if ancestor == destination else str(ancestor.relative_to(destination))][:3])
            if identity(ancestor.lstat())[:3] != pinned:
                raise GCRefused('gc_guarded_copy_parent_changed')
        pinned_parent = (parent_identity if target.parent == destination.parent else
                         created['.' if target.parent == destination else str(target.parent.relative_to(destination))][:3])
        if identity(source.lstat()) != expected:
            raise GCRefused('gc_guarded_source_changed')
        if stat.S_ISDIR(expected[2]):
            parent = _open_directory(target.parent)
            try:
                if identity(os.fstat(parent))[:3] != pinned_parent:
                    raise GCRefused('gc_guarded_copy_parent_changed')
            ...
            digests[relative], created[relative] = _copy_file(source, target, expected, check, pinned_parent)
    _same_tree(tree)
...
def execute(tree, directory, destination, check, plan):
    ...
    if destination is not None:
        ...
        manifest_identity = identity(manifests.lstat())[:3]
        _sync(directory)

        def verify():
            check()
            if identity(manifests.lstat())[:3] != manifest_identity:
                raise GCRefused('gc_guarded_manifest_directory_changed')
        ...
        verify()
        _sync(manifests)
    remove(tree, verify, copied=copied)
    verify()
