...
def prepare_interruption(tp, results_dir: Path, cfg: dict, reason: str,
    ...
    if any(identity is not None and identity[4] > MAX_RECEIPT_BYTES
           for _, identity in outputs):
        # Existing accounting uses a compatibility JSON reader. Exclude large
        # receipts here; the publisher rechecks these exact output identities.
        raise ResumeValidationError("interruption_output_limit")
