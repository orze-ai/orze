...
def _stat(path):
    # These are existing mutable launch inputs, not immutable result artifacts.
    # Match the producer's ordinary file/link semantics while pinning stat data.
    info = path.stat()
...
def _identity(folder, cfg):
    ...
    # Use the producer's nonblocking regular-file/observed-size reader, not
    # the legacy open/read-until-EOF helper. No large hashes enter a writer.
    invocation = _native()._capture(folder, cfg)
    value = invocation.identity
...
def capture_preflight_source(lake, idea_dir, cfg):
    ...
    try:
        ...
        if not enabled and row is None:
            return PreflightSourceCapture(str(folder), None, "", ())
...
def _verify(lake, folder, cfg, capture, *, consumer_ref, check_inputs):
    ...
    if source is None:
        if not enabled and row is None and capture._files == ():
            return None
        raise _hold("native_receipt_required")
    if capture._policy_sha256 != _policy(cfg):
        raise _hold("capture_changed")
    if not enabled or lake is None or row is None:
        raise _hold("native_receipt_required")
