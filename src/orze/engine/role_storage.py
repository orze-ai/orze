...
def _namespace(lease):
    _require_role_namespace(lease.lock_dir)
    _check_route(lease.route)
    _check_file(marker_path(lease.lock_dir), lease.marker)


def _require_role_namespace(lock):
    from orze.core.idea_source_lock import idea_source_lock_protected
    if idea_source_lock_protected(lock):
        raise RoleStorageHOLD('role_storage_source_namespace_protected')
...
class _Transition:
    ...
    def check(self):
        _require_role_namespace(self.lock)
...
def release_guarded_role(lease, owner, *, outcome=None, exit_code=None):
    ...
    try:
        owner._require_release(outcome, exit_code)
    except BaseException as exc:
        raise owner._hold('role_release_unconfirmed') from exc
    owner._current()
