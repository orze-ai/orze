...
def _read(path: Path, *, snapshots=None) -> bytes:
    ...
    try:
        ...
        if snapshots is not None:
            snapshots.append((path, _identity(after), hashlib.sha256(raw).digest()))
        return raw
    finally:
        os.close(fd)
...
def _scan(idea_dir: Path, *, pending: tuple[AttemptRef, str] | None = None):
    ...
    result, snapshots = {}, []
    directories = [(root, _identity(before))]
    for attempt in attempts:
        initial = _directory(attempt)
        directories.append((attempt, _identity(initial)))
        ...
        raw = _read(attempt / "prepared.json", snapshots=snapshots)
        ...
        if committed:
            actual = _decode(_read(attempt / "committed.json", snapshots=snapshots))
    ...
    # Individual stable reads are not a coherent multi-file snapshot. In-place
    # edits do not change their directory's timestamps, and could otherwise
    # combine a prepared file from one version with a confirmation from another.
    # Keep only bounded identities/digests, not all payload bytes, for this pass.
    for path, identity, digest in snapshots:
        if (hashlib.sha256(_read(path)).digest() != digest
                or _identity(path.lstat()) != identity):
            raise ValueError("effect_snapshot_changed")
    for directory, identity in directories:
        if _identity(_directory(directory)) != identity:
