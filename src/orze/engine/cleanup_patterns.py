...
def _patterns(cfg):
    ...
    for value in values:
        if (type(value) is not str or not value or len(value.encode("utf-8")) > 4096
                or len(PurePosixPath(value).parts) > 64 or "\0" in value or "\\" in value
...
def _matches(parts, pattern):
    # pathlib glob's terminal ** selects directories, never regular leaves.
...
def _declarations(folder, cfg):
    ...
    for value in cfg.get("sealed_files") or []:
        exact.add(_absolute(value, root))
    for value in cfg.get("sealed_hashes") or {}:
        exact.add(_absolute(value, root))
    cleanup = cfg.get("cleanup") or {}
    if cleanup.get("script"):
        exact.add(_absolute(cleanup["script"], root))
    for post in cfg.get("post_scripts") or []:
        if not isinstance(post, dict):
            raise CleanupRefused("cleanup_protected_configuration_invalid")
        if post.get("script"):
            exact.add(_absolute(post["script"], root))
    bundle = cfg.get("evaluation_bundle") or {}
    if isinstance(bundle, dict):
        for value in bundle.get("files") or []:
            exact.add(_absolute(value, root))
    ...
    if isinstance(resume, dict):
        ...
        for value in resume.get("immutable_inputs") or []:
            trees.add(_absolute(value, root))
    lineage = cfg.get("model_lineage") or {}
    if isinstance(lineage, dict) and lineage.get("artifact"):
        trees.add(_absolute(lineage["artifact"], folder))
    checkpoint = cfg.get("eval_checkpoint")
    if checkpoint:
        trees.add(_absolute(checkpoint, folder))
    elif cfg.get("eval_script"):
        trees.add(folder / "best_model.pt")
...
def _bound_storage(connection, folder):
    """Protect the actual route and pinned control root, not mutable cfg only."""
    if connection is None:
        return set(), set()
    databases = connection.execute("PRAGMA database_list").fetchall()
    main = [row[2] for row in databases if row[1] == "main"]
    if len(main) != 1 or not main[0] or not Path(main[0]).is_absolute():
        raise CleanupRefused("cleanup_catalog_route_invalid")
    exact = {Path(main[0] + suffix) for suffix in ("", "-wal", "-shm", "-journal")}
    ...
    exact.update(folder / item["path"] for item in binding["contract"]["outputs"].values())
    root = Path(binding["root"])
    # Native B1 factory pins control/artifacts. The adjacent control/state
    # contains durable identity slots, so preserve the original control tree.
    return exact, {root.parent if root.name == "artifacts" else root}
        ...
        try:
            ...
            with attempt_effect_lock(folder) as lease:
                with _authority(folder, cfg, lake) as connection:
                    bound_exact, bound_trees = _bound_storage(connection, folder)
                    exact.update(bound_exact)
                    trees.update(bound_trees)
