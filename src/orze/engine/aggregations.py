...
@dataclass
class _CVMixHonest(Recipe):
    """Honest 2-part mixture: ``alpha * a + (1 - alpha) * b``.

    Unlike :class:`_CVMix`, ``fit`` takes an explicit ``fit_mask`` and is
    *guaranteed* never to look at labels outside that mask. The caller
    is responsible for ensuring the mask does not overlap with any
    downstream ``report_mask`` — :func:`bundle_combiner.search` enforces
    that contract at the search boundary.
    """
    name: str = "cv_mix_honest"
    alpha: float = 0.5
    grid: int = 21
    is_fit: bool = False
    honest: bool = True
    fit_mask_signature: Optional[str] = None

    def fit(
        self,
        parts_scores: Sequence[np.ndarray],
        labels: np.ndarray,
        *,
        fit_mask: np.ndarray,
        ...
        if len(parts_scores) != 2:
            raise ValueError("cv_mix_honest currently supports exactly 2 parts")
        a = np.asarray(parts_scores[0], dtype=np.float64)
        b = np.asarray(parts_scores[1], dtype=np.float64)
        y = np.asarray(labels, dtype=np.float64)
        m = np.asarray(fit_mask, dtype=bool)
        if a.shape != b.shape or a.shape != y.shape or a.shape != m.shape:
            raise ValueError(
                "cv_mix_honest.fit: parts/labels/fit_mask shapes must match "
                f"(got {a.shape}, {b.shape}, {y.shape}, {m.shape})")
        if not m.any():
            raise ValueError("cv_mix_honest.fit: fit_mask is empty")
        if groups is not None:
            g = np.asarray(groups)
        else:
            g = np.zeros_like(y, dtype=int)

        def default_metric(s_, y_, g_):
            vals = []
            for gg in np.unique(g_):
                mm = g_ == gg
                if y_[mm].sum() == 0 or (1 - y_[mm]).sum() == 0:
                    continue
                vals.append(_auroc_like(s_[mm], y_[mm]))
            return float(np.mean(vals)) if vals else 0.5

        metric = metric or default_metric

        a_fit, b_fit, y_fit, g_fit = a[m], b[m], y[m], g[m]
        grid = np.linspace(0.0, 1.0, self.grid)
        best_alpha, best_score = 0.5, -np.inf
        for alpha in grid:
            s = alpha * a_fit + (1 - alpha) * b_fit
            try:
                score = float(metric(s, y_fit, g_fit))
            except Exception:
                continue
            if score > best_score:
        ...
        self.alpha = best_alpha
        self.is_fit = True
        # Record a signature of the fit mask so a caller attempting to
        # "report" on the very same mask can be flagged as leaky.
        try:
            self.fit_mask_signature = hashlib.sha1(
                np.ascontiguousarray(m).tobytes()
            ).hexdigest()
        except Exception:  # pragma: no cover
            self.fit_mask_signature = None
        return self

    def apply(self, parts_scores):
        if not self.is_fit:
            raise RuntimeError("cv_mix_honest.apply called before fit")
        a = np.asarray(parts_scores[0], dtype=np.float64)
        b = np.asarray(parts_scores[1], dtype=np.float64)
        return self.alpha * a + (1 - self.alpha) * b


@_register("calibrators", "cv_mix_honest")
def _mk_cv_mix_honest(**_):
    return _CVMixHonest()


def per_group_rank_normalize(x: np.ndarray, groups: np.ndarray) -> np.ndarray:
    """Per-group rank normalization. Unsupervised — no labels required.

    Within each ``groups`` level, replace scores by their rank / n_group.
    This is the standard "leakage-free" pre-mixture transform used for
    pgmAP-style per-group APs.
    """
    ...
    for gid in np.unique(g):
        ...
        if n == 0:
            continue
        # rankdata-style average ranks / n (matches champion reproducer)
        ranks = np.argsort(np.argsort(x[m], kind="stable"), kind="stable") + 1
        out[m] = ranks.astype(np.float64) / n
    return out
