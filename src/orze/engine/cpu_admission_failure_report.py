...
Only a current source validated by a fixed native adapter may authorize this action.
The CPU occurrence is NOT the claim ID: zero-allocation receipts belong
...
class FailurePolicy:
    """Internal fixed adapter policy, never selected from worker/config input."""
    source_phase: str
    hold: type
    scope: Callable
    cached: Callable
    message: str
    source_states: tuple = ("TERMINAL",)

    @property
    def report_phase(self):
        return self.source_phase + "_failure_report"

    @property
    def action_prefix(self):
        return self.source_phase.replace("_", "-") + "-failure-"

    @property
    def reason_code(self):
        return self.source_phase + "_failed"

    def error(self, suffix):
        return self.source_phase + "_failure_" + suffix


def _source(policy, tx, folder, ref, cfg):
    policy.scope(tx.lake, folder, cfg)
    row = require_current(tx.conn, ref, states=policy.source_states)
    pending = ((tx.prepared_ref, tx.prepared_sha256)
               if tx.prepare_started else None)
    if (row["binding"].get("origin") != "native_" + policy.source_phase
            or row["binding"].get("scope") != str(folder)
            or policy.cached(row, folder, pending=pending) is not False):
        raise policy.hold(policy.error("source_invalid"))
    return row


def _claim(policy, folder, binding):
    ...
    if (type(claim_id) is not str or not claim_id
        ...
        raise policy.hold(policy.error("claim_changed"))
    return claim_id, digest, claim["gpu"]


def _zero_expected(policy, ref, claim_id, gpu):
    return {"schema_version": 1, "idea_id": ref.task_id,
            ...
            "reason_code": policy.reason_code, "process_pid": None}


def _no_start(policy, folder, claim_id):
    path = folder / "_compute_receipts" / claim_id / "start.json"
    try:
        path.lstat()
    except FileNotFoundError:
        return
    raise policy.hold(policy.error("claim_already_allocated"))


def _zero_read(policy, folder, expected, payload=None):
    _no_start(policy, folder, expected["attempt_id"])
    ...
    if (set(actual) != set(expected) | {"finished_at"}
        ...
        raise policy.hold(policy.error("zero_receipt_invalid"))
    for path in (directory, directory.parent, folder):
        _sync(path)
    return hashlib.sha256(raw).hexdigest()


def _plan(policy, source, claim_id, claim_sha, expected, count):
    return {"operation": policy.report_phase, "source_attempt": source,
            "claim_attempt_id": claim_id, "claim_sha256": claim_sha,
            "zero_gpu_receipt": expected, "failure_count_after": count,
            "repair_status": _REPAIR}


def _confirmed_action(policy, folder, row):
    ...
    if (row["state"] != "TERMINAL" or terminal.get("outcome") != "reported"
        ...
        raise policy.hold(policy.error("report_unconfirmed"))
    prepared = _decode(_read_effect(
        folder / "_execution_effects" / row["attempt_id"] / "prepared.json"))
    if not same(prepared["plan"], _plan(policy,
            terminal.get("source_attempt"), terminal.get("claim_attempt_id"),
            terminal.get("claim_sha256"), terminal.get("zero_gpu_receipt"), count)):
        raise policy.hold(policy.error("report_receipt_changed"))
    return terminal, count


def _reported_files(policy, folder, terminal, expected):
    metrics, digest = _read(folder / "metrics.json")
    if (metrics.get("status") != "FAILED"
            or digest != terminal.get("metrics_sha256")
            or _zero_read(policy, folder, expected) != terminal.get("zero_gpu_receipt_sha256")):
        raise policy.hold(policy.error("report_files_changed"))


def report_failure(lake, idea_dir, ref, failure_counts, cfg, *, policy):
...
"""
    ...
    if (lake is None or not isinstance(ref, AttemptRef)
            or ref.phase != policy.source_phase or ref.task_id != folder.name):
        raise policy.hold(policy.error("reference_required"))
    memory_count = failure_counts.get(ref.task_id, 0)
    if type(memory_count) is not int or memory_count < 0:
        raise policy.hold(policy.error("counter_invalid"))
    source_identity = asdict(ref)
    action_id = policy.action_prefix + hashlib.sha256(json.dumps(
        source_identity, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    try:
        with execution_transaction(lake, folder) as tx:
            source = _source(policy, tx, folder, ref, cfg)
            tx.watch_dependency(ref)
            claim_id, claim_sha, gpu = _claim(policy, folder, source["binding"])
            if claim_id == ref.attempt_id:
                raise policy.hold(policy.error("claim_identity_invalid"))
            expected = _zero_expected(policy, ref, claim_id, gpu)
            _no_start(policy, folder, claim_id)
            binding = {"origin": "controller_action", "operation": policy.report_phase,
                       "source_attempt": source_identity,
                       "claim_attempt_id": claim_id, "claim_sha256": claim_sha}
            prior = current_attempt(tx.conn, ref.task_id, policy.report_phase)
            if prior is not None and prior["attempt_id"] == action_id:
                terminal, count = _confirmed_action(policy, folder, prior)
                ...
                if (not same(prior["binding"], binding)
                    ...
                    raise policy.hold(policy.error("duplicate_changed"))
                _reported_files(policy, folder, terminal, expected)
                tx.watch_attempt(AttemptRef(ref.task_id, policy.report_phase, action_id, prior["generation"]))
                status = "duplicate"
            else:
                if not same(_launch_state(lake, ref.task_id),
                            source["binding"].get("launch_lifecycle")):
                    raise policy.hold(policy.error("lifecycle_changed"))
                previous = 0 if prior is None else _confirmed_action(policy, folder, prior)[1]
                count = max(memory_count, previous) + 1
                action = create_attempt(tx.conn, ref.task_id, policy.report_phase, action_id, binding)
                mark_running(tx.conn, action)
                digest = tx.prepare(action, _plan(policy,
                    source_identity, claim_id, claim_sha, expected, count))
                from orze.engine.launcher import _write_failure
                payload = _write_failure(folder, policy.message, cfg=cfg, effect_lease=tx.lease)
                metrics, metrics_sha = _read(folder / "metrics.json")
                if not same(metrics, payload):
                    raise AttemptAuthorityError(policy.error("metrics_unconfirmed"))
                _sync(folder)
                receipt = accounting.record_zero_gpu_outcome(
                    ref.task_id, folder, gpu, "rejected", policy.reason_code, phase="admission")
                zero_sha = _zero_read(policy, folder, expected, receipt)
                if not lake._record_state_transition_in_tx(
                        ref.task_id, "CLAIMED", "FAILED", reason=policy.reason_code,
                        host=socket.gethostname(), pid=os.getpid(), sop_type="training"):
                    raise AttemptAuthorityError(policy.error("lifecycle_rejected"))
                ...
                if finish_attempt(tx.conn, action, terminal) != "committed":
                    raise AttemptAuthorityError(policy.error("report_not_new"))
                _reported_files(policy, folder, terminal, expected)
                status = "reported"
            if _claim(policy, folder, source["binding"]) != (claim_id, claim_sha, gpu):
                raise policy.hold(policy.error("claim_changed"))
            _source(policy, tx, folder, ref, cfg)
    except StaleAttempt:
        return {"status": "stale"}
    except TerminationUnconfirmed:
        raise
    except Exception as exc:
        raise policy.hold(policy.error("publication_unconfirmed")) from exc
