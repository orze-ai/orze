...
def _exact_row(actual, expected):
    # Python equality considers True == 1 == 1.0. Receipt identity is the
    # bounded canonical JSON text, not that looser value comparison. _row
    # already rejected noncanonical, duplicate-key, or malformed stored JSON.
    def identity(row):
        return (
            row["attempt_id"], row["task_id"], row["phase"], row["generation"],
            row["state"], row["hold_reason"], _json(row["binding"]),
            None if row["terminal"] is None else _json(row["terminal"]),
        )
...
def _update(conn, ref, before, *, state, binding=None, terminal=None, reason=None):
    ...
    if changed != 1 or not _exact_row(
            require_current(conn, ref, states=(state,)), expected):
