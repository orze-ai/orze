...
def _run_history_control(tmp_path, code):
    import json
    child = subprocess.Popen([sys.executable, '-c', BOOT + code, str(tmp_path),
        json.dumps(fixture._config(tmp_path))],
        env={**os.environ, 'PYTHONDONTWRITEBYTECODE':'1', 'PYTHONPATH':fixture.SOURCE,
             'CUDA_VISIBLE_DEVICES':''}, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    pidfd = os.pidfd_open(child.pid)
    try:
        stdout, stderr = child.communicate(timeout=15)
        assert child.returncode == 0, stdout + stderr
...
def test_closed_history_preserves_legal_aliases_and_pending_stage(tmp_path):
    _run_history_control(tmp_path, r'''
from orze.engine import controller_handoff as handoff
for name,status,state in [('queued','pending','QUEUED'),('failed','partial','FAILED'),('dead','dead','FAILED')]:
    host.lake.conn.execute("INSERT INTO ideas(idea_id,title,config,raw_markdown,status) VALUES (?, 'title','{}','',?)",(name,status))
    host.lake.conn.execute("INSERT INTO idea_state(idea_id,current_state) VALUES (?,?)",(name,state))
host.lake.conn.execute("INSERT INTO idea_stage_state(idea_id,stage,current_state,updated_at) VALUES ('queued','training','PENDING','fixture')")
host.lake.conn.commit()
session.finish()
route=handoff._Route(cfg)
digest=handoff._validate_history(route,session.ctx.controller_id,0)
assert type(digest) is str and len(digest)==64
assert handoff._validate_history(route,session.ctx.controller_id,0,expected=digest)==digest
with route.connection() as conn:
    assert conn.execute("SELECT status FROM ideas WHERE idea_id='queued'").fetchone()[0]=='pending'
    assert conn.execute("SELECT current_state FROM idea_stage_state WHERE idea_id='queued'").fetchone()[0]=='PENDING'
''')


def test_aggregate_member_limit_precedes_each_generation_drain(tmp_path):
    _run_history_control(tmp_path, r'''
from orze.engine import controller_handoff as handoff
from orze.engine.controller_members import MAX_MEMBERS
session.finish()
route=handoff._Route(cfg)
with route.connection(write=True) as conn:
    conn.executemany("INSERT INTO controller_members VALUES (?,?,?)",
        [(f'member-{number}',session.ctx.controller_id,'{}') for number in range(MAX_MEMBERS+1)])
def no_observer(*args,**kwargs):
    raise AssertionError('aggregate limit must precede per-generation drain work')
handoff._history_observer=no_observer
try: handoff._validate_history(route,session.ctx.controller_id,0)
except ControllerHOLD as exc: assert str(exc)=='controller_handoff_history_member_limit'
else: raise AssertionError('oversized aggregate history accepted')
''')


def test_borrowed_writer_history_reads_uncommitted_member_without_committing(tmp_path):
    _run_history_control(tmp_path, r'''
from orze.engine import controller_handoff as handoff
session.finish()
route=handoff._Route(cfg)
digest=handoff._validate_history(route,session.ctx.controller_id,0)
with route.connection(write=True) as conn:
    def no_new_route_connection(*args,**kwargs):
        raise AssertionError('borrowed history must not open an independent route connection')
    route.connection=no_new_route_connection
    assert handoff._validate_history(route,session.ctx.controller_id,0,expected=digest,connection=conn)==digest
    assert conn.in_transaction
    conn.execute("INSERT INTO controller_members VALUES ('writer-only',?, '{}')",(session.ctx.controller_id,))
    try: handoff._validate_history(route,session.ctx.controller_id,0,expected=digest,connection=conn)
    except ControllerHOLD as exc: assert str(exc)=='controller_stop_members_changed'
    else: raise AssertionError('borrowed writer member change was invisible to drain verification')
    assert conn.in_transaction
    assert conn.execute("SELECT COUNT(*) FROM controller_members WHERE member_id='writer-only'").fetchone()[0]==1
    conn.rollback()
    assert handoff._validate_history(route,session.ctx.controller_id,0,expected=digest,connection=conn)==digest
    assert not conn.in_transaction
    assert conn.execute("SELECT COUNT(*) FROM controller_members WHERE member_id='writer-only'").fetchone()[0]==0
''')
