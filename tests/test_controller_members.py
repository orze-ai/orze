...
COMMON = r'''
import contextlib, hashlib, json, os, pathlib, select, signal, sqlite3, subprocess, sys
...
'''
    ...
    try:
        ...
        assert child.returncode == 0, stderr.decode()[-16000:]
...
assert rows()[0]['action_state']!='SETTLED'
try: members.settle_process(p, outcome='completed')
except (control.ControllerHOLD, primitive.SupervisionUncertain): pass
else: raise AssertionError('uncertain process settled')
''')
...
def test_popen_creates_owned_supervisor_then_raises_is_not_no_execution(tmp_path):
    run_case(tmp_path, r'''
original=primitive.subprocess.Popen
created=[]
def response_lost(*args,**kwargs):
    child=original(*args,**kwargs); created.append((child,os.pidfd_open(child.pid)))
    raise OSError('injected after actual Popen returned')
primitive.subprocess.Popen=response_lost
try:
    try: prepare()
    except (OSError, primitive.SupervisionUncertain, control.ControllerHOLD): pass
    else: raise AssertionError('Popen response loss swallowed')
    assert len(created)==1
    assert rows()[0]['os_state']!='NO_EXECUTION'
    assert rows()[0]['action_state']!='SETTLED'
finally:
    # Parent closed its private socket; the real owned supervisor observes EOF
    # before config/worker creation. Wait exactly this captured Popen, no scan.
    for child,descriptor in created:
        try:
...
def test_native_stop_zero_settles_as_interrupted_not_completed(tmp_path):
    run_case(tmp_path, r'''
folder=scope/'idea-one'; folder.mkdir()
with authority.execution_transaction(lake, folder) as tx:
    ref=create_attempt(tx.conn,'idea-one','training','attempt-one',{})
    tx.watch_attempt(ref)
p=prepare('import os,time;\nif os.fork(): os._exit(0)\ntime.sleep(30)',
    ident={'attempt_ref':asdict(ref),'scope':str(folder)})
with authority.execution_transaction(lake,folder) as tx:
    mark_running(tx.conn,ref,{'process_supervision':p.binding}); tx.watch_attempt(ref)
p.start()
import time
time.sleep(.1)
...
assert p.closure_receipt()['stop_requested'] is True
with authority.execution_transaction(lake,folder) as tx:
    digest=tx.prepare(ref,{'operation':'owned stop result'})
    finish_attempt(tx.conn,ref,{'outcome':'interrupted','effect_receipt_sha256':digest})
assert rows()[0]['action_state']=='SETTLED'
assert rows()[0]['outcome']=='interrupted'
''')


def test_role_intent_unknown_holds_controller_before_member_was_registered(tmp_path):
    run_case(tmp_path, r'''
from orze.core.fs import _fs_lock
from orze.engine import role_supervision as roles
lock=base/'role-lock'; assert _fs_lock(lock)
publish=roles._publish
def lost_response(*args, **kwargs):
    publish(*args, **kwargs)
    raise OSError('injected after actual role INTENT publication')
roles._publish=lost_response
try:
    roles.begin_role_launch({'role_name':'engineer','attempt_id':'role-one','scope':str(scope),
        'lock_dir':str(lock),'nonce_sha256':'1'*64,'command_sha256':'2'*64,
        'trigger_delivery':None,'trigger_delivery_db':None})
except roles.RoleSupervisionHOLD as error:
    assert error.owner is not None
else: raise AssertionError('lost role receipt response accepted')
assert (lock/'role-process.json').exists()
assert not members._OWNERS
