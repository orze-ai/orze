...
sys.argv=['orze','stop','-c',sys.argv[1],'--timeout','8']
raise SystemExit(main())
'''


def test_actual_cli_stop_zero_requires_registered_controller_exit(product_process):
    c = product_process("normal")
    assert c.event("constructed")["lake"] == "IdeaLake"
    c.worker()
    c.wait(lambda: any(row["state"] == "TERMINAL" for row in c.rows("execution_attempts")))
    observer = subprocess.Popen([sys.executable, "-c", CLI_OBSERVER, str(c.root / "orze.yaml")],
        cwd=c.root, env=c.environment, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    c.observers.append(observer)
    c.capture(observer.pid)
    stdout, stderr = observer.communicate(timeout=13)
    assert observer.returncode == 0, (stdout.decode(), stderr.decode())
    assert not _alive(c.controller_fd), "CLI zero alone is not captured controller exit"
    assert c.child.wait(timeout=5) == 0
    sessions = c.rows("controller_sessions")
    assert len(sessions) == 1 and sessions[0]["request_json"] and sessions[0]["ack_json"]
    assert all(row["state"] == "TERMINAL" for row in c.rows("execution_attempts"))
