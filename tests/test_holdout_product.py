...
def test_snapshot_references_are_unique_even_for_repeated_step_labels(monkeypatch, tmp_path):
    from examples.holdout import testing
    monkeypatch.setattr(testing, "_database", lambda root: {"source": str(root)})
    run = {"root": tmp_path, "snapshots": []}
    first = testing._snapshot(run, "same-step:before")
    second = testing._snapshot(run, "same-step:before")
    assert first["label"] != second["label"]
    assert run["snapshots"] == [first, second]
    assert first["database"] == second["database"] == {"source": str(tmp_path)}
