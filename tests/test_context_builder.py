...
@pytest.fixture
def lake(tmp_path):
    database = tmp_path / ".orze" / "idea_lake.db"
    database.parent.mkdir()
    instance = IdeaLake(str(database))
    try:
        yield instance
    finally:
        instance.close()
...
def _write_idea(results, idea_id, metrics, *, config=None, source=None,
    ...
    if lake is not None:
        lake.insert(
            ...
            status=str(metrics["status"]).lower(),
        )
    return idea_dir
...
def test_digest_ranks_exact_qualified_source_not_raw_metric(tmp_path, lake):
...
def test_generic_valid_half_score_is_not_hardcoded_as_sentinel(tmp_path, lake):
    results = tmp_path / "results"
    _write_idea(
        results,
        "idea-valid-half",
...
def test_cached_historic_leader_is_requalified_beyond_recent_window(tmp_path, lake):
    results = tmp_path / "results"
    old = _write_idea(
        results,
        "idea-historic-best",
...
def test_redirected_metric_artifact_cannot_enter_digest(tmp_path, lake):
    results = tmp_path / "results"
    ...
    lake.insert(
        "idea-redirected", "redirected", "{}", "", status="completed",
        eval_metrics={"status": "COMPLETED", "score": 100.0},
    )
    completed_ids, reason = authoritative_completed_idea_ids(lake.db_path)
    assert reason == "authoritative_lifecycle_loaded"
    assert "idea-redirected" in completed_ids
    (results / "_leaderboard.json").write_text(json.dumps({
        "metric": "score",
...
def test_digest_does_not_export_error_or_config_values(tmp_path, lake):
    results = tmp_path / "results"
