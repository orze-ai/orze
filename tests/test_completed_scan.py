...
def test_changes_after_a_page_reject_whole_scan(tmp_path, mutation):
    ...
    with pytest.raises(CompletedScanUnavailable) as error:
        with CompletedIdeaScan(path, page_size=2) as scan:
            ...
            if mutation in {"status", "state", "stage", "drop_stage", "unrelated", "change_restore"}:
                sql = {
                    ...
                    "change_restore": "UPDATE ideas SET status='failed'",
                }[mutation]
                with sqlite3.connect(path, timeout=0) as writer:
                    writer.execute(sql)
                    if mutation == "change_restore":
                        writer.commit()
                        writer.execute("UPDATE ideas SET status='completed'")
...
def test_sqlite_no_op_commit_without_revision_change_preserves_scan(tmp_path):
    path = lake(tmp_path)
    with CompletedIdeaScan(path, page_size=2) as scan:
        pages = scan.pages()
        first = next(pages)
        with sqlite3.connect(path) as writer:
            writer.execute("UPDATE ideas SET status=status")
        assert len(first) + sum(map(len, pages)) == 5
