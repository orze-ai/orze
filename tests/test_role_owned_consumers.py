...
def _delivery_state(db_path, delivery_id):
    with sqlite3.connect(db_path) as connection:
        return connection.execute(
            "SELECT state FROM trigger_deliveries WHERE delivery_id=?", (delivery_id,)
        ).fetchone()[0]


@pytest.mark.parametrize("route", ["shutdown", "kill_all", "atexit", "upgrade"])
def test_pending_role_survives_every_consumer_without_inventing_closure(project, route, monkeypatch):
    p = project
    owner = p.begin()
    ...
    if route in {"shutdown", "kill_all"}:
        ...
        with pytest.raises(role_supervision.RoleSupervisionHOLD):
            manager._restart_in_place(controller._kill_and_save, remove_pid)
...
def test_closed_tree_waits_for_actual_trigger_settlement_before_finished(project, monkeypatch):
    ...
    try:
        ...
        assert _delivery_state(lake.db_path, launch["delivery_id"]) == "STARTED"
        ...
        assert roles.check_active_roles(active) == []
        assert _delivery_state(lake.db_path, launch["delivery_id"]) == "TERMINAL"
    finally:
        lake.close()
