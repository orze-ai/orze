...
class TestProbeMechanisms:
    def test_real_file_directory_success_and_collision_preservation(self, tmp_path, monkeypatch):
        ...
        def observed(source_fd, source_name, target_fd, target_name):
            source, target = _FdEntry(source_fd, source_name), _FdEntry(target_fd, target_name)
        ...
        monkeypatch.setattr(storage, 'rename_no_replace_at', observed)
    ...
    def test_parent_replaced_during_operation_never_cleans_replacement(self, tmp_path, monkeypatch):
        ...
        def replace(source_fd, source_name, target_fd, target_name):
            root.rename(old)
            root.mkdir()
            (root / 'foreign').write_bytes(b'keep')
            return actual(source_fd, source_name, target_fd, target_name)

        monkeypatch.setattr(storage, 'rename_no_replace_at', replace)
    ...
    def test_wrong_overwriting_primitive_cannot_pass_collision_control(self, tmp_path, monkeypatch):
        ...
        def overwrite(source_fd, source_name, target_fd, target_name):
            os.replace(source_name, target_name, src_dir_fd=source_fd, dst_dir_fd=target_fd)

        monkeypatch.setattr(storage, 'rename_no_replace_at', overwrite)
        with pytest.raises(storage.StoragePreflightError):
            storage.require_atomic_rename_support(tmp_path)
    ...
    def test_deployment_gc_uses_project_relative_route_not_cwd(self, tmp_path, unsupported_root, monkeypatch):
        from orze.engine.storage_preflight import require_deployment_storage, StoragePreflightError
        results = unsupported_root / 'results'
        results.mkdir()
        monkeypatch.chdir(tmp_path)
        cfg = {'_project_root': str(unsupported_root),
               'gc': {'enabled': True, 'checkpoints_dir': 'checkpoints'}}
        with pytest.raises(StoragePreflightError, match='storage_atomic_rename_unsupported'):
            require_deployment_storage(cfg, results)
        assert not (tmp_path / 'checkpoints').exists()

    @pytest.mark.parametrize('marker_name', ['.orze_disabled', '.orze_stop_all', '.orze_shutdown'])
    def test_existing_stop_precedes_storage_directory_or_probe_creation(self, tmp_path, marker_name):
        ...
        marker = runner.results_dir / marker_name
        marker.write_bytes(b'keep original operator stop')
        with pytest.raises(ControllerStopHOLD):
            Orze.run(runner)
        assert not (tmp_path / '.orze').exists()
        assert marker.read_bytes() == b'keep original operator stop'
        assert events == []


class _FdEntry:
    """Test-only view of an owned FD/name; never resolves /proc or a pathname."""
    ...
    def read_bytes(self):
        fd = os.open(self.name, os.O_RDONLY | os.O_NOFOLLOW, dir_fd=self.parent)
        try:
