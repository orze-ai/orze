...
def test_exact_match_does_not_skip_candidate_capacity_refusal(lake, count, expected):
    lake.conn.executemany(
        "INSERT INTO ideas(idea_id,title,config,raw_markdown,status) VALUES (?,'History','seed: 13\n','','completed')",
...
def test_unavailable_preceding_config_is_not_hidden_by_later_exact_match(lake, unavailable):
    lake.conn.execute(
        "INSERT INTO ideas(idea_id,title,config,raw_markdown,status) VALUES ('idea-unknown','Unknown',?,'','completed')",
        (unavailable,),
    )
    lake.conn.commit()
