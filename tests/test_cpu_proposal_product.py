...
def test_policy_normal_duplicate_and_conflict_results_survive_restarts(project, monkeypatch):
    root, cfg, run = project
    selected = [proposal("idea-first", request("pass"))]
    seen = []
    ...
    register(monkeypatch, cfg, Policy)
    assert run() == 0
    selected[0] = proposal("idea-copy", request("pass"))
    assert run() == 0
    assert run() == 0
    outcomes = {item["request_id"]: item for item in seen[-1]["recorded_proposals"]["results"]}
    assert len(outcomes) == 2
    assert outcomes["plan-idea-copy"]["status"] == "config_duplicate"
    assert outcomes["plan-idea-copy"]["existing_id"] == "idea-first"
    assert read(root, "SELECT idea_id,status FROM ideas") == [("idea-first", "queued")]
    selected[0] = {**selected[0], "reason": "changed same stable request key"}
    assert run() == 75
    assert read(root, "SELECT idea_id,status FROM ideas") == [("idea-first", "queued")]
    assert read(root, "SELECT COUNT(*) FROM cpu_action_reservations") == [(0,)]
    assert read(root, "SELECT name FROM sqlite_master WHERE name='execution_attempts'") == []
    assert not (root / "results" / "idea-copy" / "claim.json").exists()


def test_proposal_does_not_bypass_later_execution_wall_budget(project, monkeypatch):
    root, cfg, run = project
    ...
    class Policy:
        ...
        def decide(self, snapshot, budget):
            return proposal("idea-too-expensive", request("raise AssertionError('must not execute')"))

    register(monkeypatch, cfg, Policy)
    assert run() == 0
    cfg["action_policy"]["kind"] = "queue"
    assert run() == 0
    assert read(root, "SELECT idea_id,status FROM ideas") == [("idea-too-expensive", "queued")]
    assert read(root, "SELECT COUNT(*) FROM cpu_action_reservations") == [(0,)]
    assert read(root, "SELECT name FROM sqlite_master WHERE name='execution_attempts'") == []
    ...
    assert not (root / "results" / "idea-too-expensive" / "claim.json").exists()


def test_normal_exact_task_and_identity_conflict_are_distinct_durable_outcomes(project, monkeypatch):
    root, cfg, run = project
    selected = [proposal("idea-shared", request("pass"))]
    seen = []
    ...
    register(monkeypatch, cfg, Policy)
    assert run() == 0
    selected[0] = {**selected[0], "request_id": "another-key-exact-task"}
    assert run() == 0
    selected[0] = {**selected[0], "request_id": "another-key-conflicting-task",
                   "domain_request": request("print('different question')")}
    assert run() == 0
    assert run() == 0
    statuses = {item["request_id"]: item["status"]
                for item in seen[-1]["recorded_proposals"]["results"]}
    assert statuses == {"plan-idea-shared": "inserted",
                        "another-key-exact-task": "already_present_exact",
                        "another-key-conflicting-task": "conflict"}
    assert read(root, "SELECT idea_id,status FROM ideas") == [("idea-shared", "queued")]
    assert read(root, "SELECT COUNT(*) FROM idea_transitions") == [(1,)]
    assert read(root, "SELECT COUNT(*) FROM cpu_action_reservations") == [(0,)]
