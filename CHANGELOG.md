...
- **Benchmark reuse is now explicitly scoped and budgeted.** Contracted
  ...
  its recorded history. The ledger now lives in fixed project state, migrates
  compatible result-local history, rejects conflicting copies, survives both
  reset modes, and follows a campaign across results-directory changes.
  Corrupt, missing, duplicated, or backdated exposure evidence invalidates the
  result. Reports, caches, and notifications expose the scope, selection mode,
  remaining budget, and benchmark-fitted status so adaptive test reuse cannot
  masquerade as an independent confirmation.
